...
- 이미지를 숨기는 것은 로딩에 실패했거나, 아직 로드되지 않았는데 받을 소스(`src`·`srcSet`)도 없을 때뿐입니다. `srcSet`만 지정한 반응형 이미지도 정상 동작합니다.
- 스크린리더가 읽는 내용은 그대로입니다. 이미지는 로드가 끝난 뒤에 접근성 트리에 노출되므로, 로딩 중에 `alt`와 `fallback`이 함께 낭독되지 않습니다.
