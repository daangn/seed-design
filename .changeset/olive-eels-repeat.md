...
문서 사이트 구조가 바뀌면서 `components`, `foundations`, `patterns`, `updates`가 최상위 섹션으로 분리됐는데, CLI가 읽는 문서 목록이 예전 5개 섹션에 머물러 있었습니다. 약 80개 문서를 `docs` 명령어로 찾을 수 없었고, 이제 213개 문서가 모두 조회됩니다.
