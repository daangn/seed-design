...
- `seed-design docs read /foundations/color`처럼 새 섹션 주소가 동작합니다.
- `get-started`가 카테고리로 잡힙니다. 이전에는 목록에 아예 없었습니다.
- 카테고리 개요 문서를 `seed-design docs read /react`로 볼 수 있습니다. React·Lynx·Breeze·AI Integration·Get Started의 개요는 이전까지 문서 목록·llms-full.txt·CLI 어디에도 나오지 않았습니다.
...
**동작 변경**: `/components/{name}`은 디자인 스펙 문서로 연결됩니다. React 구현 문서는 `/react/components/{name}`으로 조회해주세요.
