...
- 모션 선호도를 따르는 `$scale.s95`~`$scale.s98` 토큰을 추가합니다. 웹(`@seed-design/css`)에서는 `prefers-reduced-motion`이 적용된 환경에서 축소 없이 `1`로 동작합니다. Lynx(`@seed-design/lynx-css`)는 `prefers-reduced-motion` 미디어 쿼리를 평가하지 않아, 해당 가드 없이 항상 축소가 적용됩니다.
