...
- `seed-design docs search "액션 버튼"`: 문서의 제목·본문·`whenToRead`를 검색해 관련도 순으로 최대 20개 문서를 출력합니다.
- `seed-design docs read /react/components/action-button`: 링크 대신 Markdown 본문을 stdout에 출력합니다. `--raw`는 제거합니다.

목록과 검색 결과는 주소를 첫 필드로 두고 제목·설명·`whenToRead`·deprecated 여부를 한 줄에 표시합니다. 문서 주소는 `docs read`에, `/`로 끝나는 폴더 주소는 `docs list`에 넘깁니다. 검색 결과의 `#앵커`를 읽으면 해당 문서 전체를 반환합니다.
...
- `search_docs({ query: "액션 버튼" })`를 추가합니다. CLI와 같은 색인·검색 로직을 사용해 관련 문서를 최대 20개 반환합니다. 결과에는 주소·제목·설명·`whenToRead`·deprecated 여부가 포함됩니다.
...
- `list_docs`와 `get_doc`은 사이트의 문서 인덱스에서 최신 섹션·문서 목록을 읽습니다. 디자인 스펙·파운데이션·패턴·업데이트·시작하기 섹션도 조회할 수 있습니다. `list_docs`는 경로순 목록에 설명·`whenToRead`·deprecated 여부를 표시하며, `category` 필터를 제거합니다.
