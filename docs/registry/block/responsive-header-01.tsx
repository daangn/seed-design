"use client";

import type * as React from "react";
import {
  IconArrowUpRightLine,
  IconChevronDownSmallLine,
  IconHorizline3VerticalLine,
  IconMagnifyingglassFill,
  IconXmarkLine,
} from "@karrotmarket/react-monochrome-icon";
import {
  Box,
  Header as SeedHeader,
  HStack,
  Icon,
  SidePanel as SeedSidePanel,
  SuffixIcon,
  Text,
  VStack,
} from "@seed-design/react";
import { AccordionContent, Accordion, AccordionItem, AccordionTrigger } from "../ui/accordion";
import { ActionButton } from "../ui/action-button";
import { MenuContent, MenuItem, MenuRoot, MenuTrigger } from "../ui/menu";
import {
  SidePanelBody,
  SidePanelContent,
  SidePanelFooter,
  SidePanelRoot,
  SidePanelTrigger,
} from "../ui/side-panel";

type NavItem = {
  id: string;
  label: string;
  href?: string;
  subItems?: { label: string; href: string; description?: string; external?: boolean }[];
};

const NAV_ITEMS: NavItem[] = [
  { id: "used", label: "중고거래", href: "/used" },
  {
    id: "realty",
    label: "부동산",
    subItems: [
      { label: "부동산 검색", href: "/realty/search", description: "전국 매물을 한눈에" },
      {
        label: "중개사 서비스",
        href: "/realty/agents",
        description: "공인중개사 협력",
        external: true,
      },
      {
        label: "중개사 이용 가이드",
        href: "/realty/guide",
        description: "처음이라면 가이드부터",
        external: true,
      },
    ],
  },
  {
    id: "car",
    label: "중고차",
    subItems: [
      { label: "중고차 검색", href: "/car/search", description: "직거래 매물 찾기" },
      { label: "딜러 입점하기", href: "/car/dealer", description: "전문 판매자 등록", external: true },
    ],
  },
  {
    id: "jobs",
    label: "알바/과외/레슨",
    subItems: [
      { label: "알바 찾기", href: "/jobs/parttime", description: "동네 알바 모아보기" },
      { label: "과외 찾기", href: "/jobs/tutoring", description: "원하는 분야의 과외" },
      { label: "레슨 찾기", href: "/jobs/lessons", description: "취미부터 자격증까지" },
    ],
  },
  { id: "biz", label: "동네업체", href: "/biz" },
  { id: "story", label: "동네생활", href: "/story" },
  { id: "group", label: "모임", href: "/group" },
  { id: "cafe", label: "카페", href: "/cafe" },
];

function DaangnLogo(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="57"
      height="32"
      viewBox="0 0 57 32"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g clipPath="url(#daangn-responsive-header-logo-clip)">
        <path
          d="M8.2061 10.3566C3.67422 10.3566 0 13.9495 0 18.4855C0 24.7529 8.22967 28.0786 8.2061 28.0696C8.18365 28.0786 16.4122 24.7529 16.4122 18.4855C16.4122 13.9529 12.738 10.3566 8.2061 10.3566ZM8.2061 21.7764C7.59158 21.776 6.99099 21.5933 6.48027 21.2515C5.96954 20.9097 5.57162 20.4241 5.33681 19.8561C5.102 19.2881 5.04085 18.6632 5.16109 18.0604C5.28133 17.4577 5.57757 16.9041 6.01233 16.4698C6.4471 16.0354 7.00088 15.7397 7.60365 15.6201C8.20643 15.5005 8.83112 15.5623 9.39876 15.7978C9.9664 16.0332 10.4515 16.4318 10.7927 16.943C11.1339 17.4541 11.3159 18.055 11.3157 18.6697C11.3164 19.0784 11.2365 19.4833 11.0805 19.8611C10.9245 20.2389 10.6955 20.5822 10.4067 20.8713C10.1178 21.1604 9.77474 21.3897 9.39712 21.546C9.01951 21.7023 8.61477 21.7825 8.2061 21.782V21.7764Z"
          fill="#FF6F0F"
        />
        <path
          d="M10.0539 0C8.13874 0 6.80511 1.33836 6.59519 2.91924C4.04242 2.21413 2.04198 4.1543 2.04198 6.28759C2.04198 7.92125 3.16456 9.24052 4.66097 9.66942C5.86775 10.0141 8.04107 9.757 8.04107 9.757C8.02985 9.22929 8.51593 8.64994 9.26581 8.12223C11.3987 6.62219 13.0669 5.91483 13.324 3.86912C13.5934 1.72909 12.0049 0 10.0539 0Z"
          fill="#00A05B"
        />
        <path
          d="M32.6998 13.1826C30.9665 13.7272 27.9659 14.0169 25.1471 14.0169V10.5879H30.9688V8.03467H22.0251V16.7295C26.9163 16.7295 31.1866 16.1401 32.9894 15.582L32.6998 13.1826Z"
          fill="#FF6F0F"
        />
        <path
          d="M36.8096 7.0174H33.6922V17.0135H36.8096V13.8799H39.3242V11.1526H36.8096V7.0174Z"
          fill="#FF6F0F"
        />
        <path
          d="M31.0193 16.99C27.7166 16.99 25.2941 18.81 25.2941 21.4811C25.2941 24.1522 27.7189 25.9723 31.0193 25.9723C34.3197 25.9723 36.7445 24.1511 36.7445 21.4811C36.7445 18.8111 34.3219 16.99 31.0193 16.99ZM31.0193 23.4718C29.5307 23.4718 28.4138 22.7577 28.4138 21.4766C28.4138 20.1955 29.5307 19.4826 31.0193 19.4826C32.5078 19.4826 33.6248 20.1967 33.6248 21.4766C33.6248 22.7566 32.509 23.4718 31.0193 23.4718Z"
          fill="#FF6F0F"
        />
        <path d="M45.3952 18.316H42.2081V25.0785H54.8754V22.4018H45.3952V18.316Z" fill="#FF6F0F" />
        <path
          d="M54.4802 7.85162H42.0644V10.5283H51.3504C51.3504 11.193 51.3819 12.7739 51.0776 14.5704H39.6901V17.3392H56.978V14.5749H53.8976C54.3107 12.3046 54.4286 10.5351 54.4802 7.85162Z"
          fill="#FF6F0F"
        />
      </g>
      <defs>
        <clipPath id="daangn-responsive-header-logo-clip">
          <rect width="56.9825" height="32" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
}

function DesktopNavItem({ item }: { item: NavItem }) {
  if (!item.subItems) {
    return <SeedHeader.ActionButton size="small">{item.label}</SeedHeader.ActionButton>;
  }

  return (
    <MenuRoot>
      <MenuTrigger asChild>
        <SeedHeader.ActionButton size="small">
          {item.label}
          <SuffixIcon svg={<IconChevronDownSmallLine />} />
        </SeedHeader.ActionButton>
      </MenuTrigger>
      <MenuContent>
        {item.subItems.map((sub) => (
          <MenuItem key={sub.label} label={sub.label} description={sub.description} />
        ))}
      </MenuContent>
    </MenuRoot>
  );
}

function MobilePanelHeader() {
  return (
    <Box
      as="header"
      display="flex"
      alignItems="center"
      justifyContent="space-between"
      height="x16"
      paddingX="x4"
      bg="bg.layerDefault"
      flexShrink={0}
    >
      <DaangnLogo aria-label="당근" />
      <SeedSidePanel.CloseButton
        aria-label="닫기"
        style={{ position: "static", width: 40, height: 40 }}
      >
        <Icon svg={<IconXmarkLine />} size="24px" />
      </SeedSidePanel.CloseButton>
    </Box>
  );
}

function MobileNavLink({ item }: { item: NavItem }) {
  return (
    <Box
      asChild
      display="flex"
      alignItems="center"
      minHeight="60px"
      paddingX="x4"
      style={{ textDecoration: "none" }}
    >
      <a href={item.href}>
        <Text textStyle="t7Bold" color="fg.neutral">
          {item.label}
        </Text>
      </a>
    </Box>
  );
}

function MobileSubLink({ item }: { item: NonNullable<NavItem["subItems"]>[number] }) {
  return (
    <Box
      asChild
      display="flex"
      alignItems="center"
      minHeight="46px"
      paddingX="x4"
      color="fg.neutralMuted"
      style={{ textDecoration: "none" }}
    >
      <a href={item.href}>
        <HStack gap="x1" alignItems="center">
          <Text textStyle="t5Medium" color="fg.neutralMuted">
            {item.label}
          </Text>
          {item.external && <Icon svg={<IconArrowUpRightLine />} size="14px" />}
        </HStack>
      </a>
    </Box>
  );
}

function MobileNavItem({ item }: { item: NavItem }) {
  if (!item.subItems) {
    return <MobileNavLink item={item} />;
  }

  return (
    <AccordionItem value={item.id}>
      <AccordionTrigger title={item.label} />
      <AccordionContent>
        <VStack paddingBottom="x2">
          {item.subItems.map((sub) => (
            <MobileSubLink key={sub.label} item={sub} />
          ))}
        </VStack>
      </AccordionContent>
    </AccordionItem>
  );
}

export default function DaangnResponsiveHeader() {
  return (
    <SeedHeader.Root divider>
      <SeedHeader.Left>
        <DaangnLogo aria-label="당근" />
        <Box display={{ base: "none", md: "flex" }}>
          <HStack gap="x2" px="x4">
            {NAV_ITEMS.map((item) => (
              <DesktopNavItem key={item.id} item={item} />
            ))}
          </HStack>
        </Box>
      </SeedHeader.Left>
      <SeedHeader.Right>
        <Box display={{ base: "none", md: "flex" }}>
          <SeedHeader.ActionButton size="small" aria-label="검색">
            <Icon svg={<IconMagnifyingglassFill />} size="20px" />
          </SeedHeader.ActionButton>
          <ActionButton variant="neutralSolid" size="small">
            앱 다운로드
          </ActionButton>
        </Box>
        <Box hideFrom="md">
          <SidePanelRoot direction="left">
            <SidePanelTrigger asChild>
              <SeedHeader.ActionButton size="small" aria-label="메뉴 열기">
                <Icon svg={<IconHorizline3VerticalLine />} size="20px" />
              </SeedHeader.ActionButton>
            </SidePanelTrigger>
            <SidePanelContent
              aria-label="메뉴"
              showCloseButton={false}
              width="100vw"
              maxWidth="100vw"
            >
              <MobilePanelHeader />
              <SidePanelBody style={{ paddingInline: 0 }}>
                <Accordion size="large" defaultValues={["realty"]}>
                  <VStack gap="x0" paddingY="x2">
                    {NAV_ITEMS.map((item) => (
                      <MobileNavItem key={item.id} item={item} />
                    ))}
                  </VStack>
                </Accordion>
              </SidePanelBody>
              <SidePanelFooter style={{ padding: "var(--seed-dimension-x4)" }}>
                <ActionButton variant="neutralSolid" size="large">
                  앱 다운로드
                </ActionButton>
              </SidePanelFooter>
            </SidePanelContent>
          </SidePanelRoot>
        </Box>
      </SeedHeader.Right>
    </SeedHeader.Root>
  );
}
