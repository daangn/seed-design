...
- 외부 데이터(토큰 아티팩트·아이콘 데이터·카탈로그 매니페스트)를 읽는 핸들러는 데이터를 인자로 받는 `create...Handler(data)`와, 실제 데이터를 물린 기본 인스턴스를 함께 내보낸다. 테스트는 합성 데이터로 만든다.
...
- `remarkApplyLlmsFilter`는 태그를 만들어내는 플러그인들보다 뒤, `remarkLlms` 바로 앞에 있어야 한다. 앞에 두면 `remarkAutoTypeTable`이 생성하는 `<TypeTable>` 같은 태그에 `remove`가 걸리지 않는다.
