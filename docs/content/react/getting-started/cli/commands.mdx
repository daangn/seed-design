...
문서 링크와 llms.txt 링크를 조회하는 명령어예요.

```package-install
npx @seed-design/cli@latest docs [query]
```

`query`를 생략하면 카테고리 목록을 보여줘요. 아무것도 묻지 않으므로 스크립트나 에이전트에서
그대로 쓸 수 있어요.
...
npx @seed-design/cli@latest docs react/components   # 그 섹션의 항목 목록
```

문서 하나로 좁혀지면 링크를 출력해요.
...
<Callout type="info">
  ...
  `--framework`(또는 설정 파일의 `framework`)를 지정했다면 카테고리 없이 이름만
  써도 해당 프레임워크 문서로 찾아가요.

  ```sh
  npx @seed-design/cli@latest docs action-button --framework react
  ```
</Callout>

각 카테고리의 개요 문서는 `overview`로 조회해요.

```package-install
npx @seed-design/cli@latest docs react/overview
```

### 후보가 여러 개일 때

쿼리가 문서 하나로 좁혀지지 않으면 후보 경로를 출력하고 종료 코드 `2`로 끝나요. 출력된
경로 중 하나를 그대로 다시 넘기면 돼요.

```sh copy
$ npx @seed-design/cli@latest docs action
components/action-button
components/action-chip
react/components/action-button
react/components/action-chip

$ echo $?
2
```

후보 경로는 stdout으로, 왜 목록이 나왔는지 알려주는 설명은 stderr로 나가요. 그래서
`docs action | head`처럼 경로만 이어 쓸 수 있어요.
...
해결 힌트: `seed-design docs react`로 목록을 확인해보세요.
```
...
`--raw`는 항상 문서 내용만 stdout으로 내보내요. 문서 하나로 좁혀지지 않으면 후보 경로는
stderr로 나가고 stdout은 비어 있어요.
...
| 스트림 | 담는 것 |
|--------|---------|
| stdout | 질의에 대한 답. 기본 모드에서는 경로와 링크, `--raw`에서는 문서 본문 |
| stderr | 그 밖의 전부. 오류, 후보가 여러 개인 이유, 텔레메트리 안내 |

### 종료 코드

| 코드 | 의미 |
|------|------|
| `0` | 문서 하나를 찾았거나, 지정한 카테고리·섹션의 목록을 출력했어요 |
| `1` | 일치하는 문서가 없거나, 그 밖의 오류가 났어요 |
| `2` | 여러 문서가 일치해서 하나로 좁히지 못했어요 |
...
| `-u, --baseUrl <baseUrl>` | 레지스트리 base URL |
| `-f, --framework <framework>` | 프레임워크 (`react` 또는 `lynx`) |
| `--verbose` | 실패 시 stack trace 등 상세 오류 정보 출력 |
| `-h, --help` | 도움말 출력 |
