---
title: Commands
description: "@ride-developer/cli 명령어를 안내해요."
---
...
- `add`, `add-all` 실행 시 스니펫이 요구하는 `@ride-developer/react`, `@ride-developer/css` 버전과 현재 프로젝트 버전이 다르면 경고를 보여줘요.
...
```package-install
npx @ride-developer/cli@latest init
```
...
```package-install
npx @ride-developer/cli@latest init --yes
```
...
```package-install
npx @ride-developer/cli@latest add [...item-ids]
```

`item-ids`를 생략하면, 다운로드 가능한 항목을 인터랙티브하게 선택할 수 있어요.

```package-install
npx @ride-developer/cli@latest add
```
...
```package-install
npx @ride-developer/cli@latest add ui:action-button ui:alert-dialog
```
...
```package-install
npx @ride-developer/cli@latest add --baseUrl https://1-0.seed-design.pages.dev
```

```package-install
npx @ride-developer/cli@latest add --baseUrl https://1-1.seed-design.pages.dev
```
...
```package-install
npx @ride-developer/cli@latest add-all [...registry-ids]
```

모든 레지스트리의 모든 항목을 추가하려면 `--all` 옵션을 사용하세요.

```package-install
npx @ride-developer/cli@latest add-all --all # 모든 레지스트리의 모든 항목 추가
```

특정 레지스트리의 모든 항목을 추가할 수도 있습니다.

```package-install
npx @ride-developer/cli@latest add-all breeze # breeze 레지스트리의 모든 항목 추가
```

deprecated 항목을 포함하려면 `--include-deprecated` 옵션을 사용하세요.

```package-install
npx @ride-developer/cli@latest add-all ui --include-deprecated # ui 레지스트리의 모든 항목 추가 (deprecated 포함)
```
...
현재 프로젝트의 `@ride-developer/react`, `@ride-developer/css` 버전과 스니펫 요구 버전이 호환되는지 검사해요.

```package-install
npx @ride-developer/cli@latest compat
```
...
```package-install
npx @ride-developer/cli@latest compat ui:action-button ui:alert-dialog
```

컴포넌트 shorthand를 쓸 때는 `-c` 옵션을 사용해요.

```package-install
npx @ride-developer/cli@latest compat -c action-button -c alert-dialog
```

모든 registry 항목을 검사하려면 `--all`을 사용하세요.

```package-install
npx @ride-developer/cli@latest compat --all
```
