...
- `add`, `add-all` 실행 시 스니펫이 요구하는 패키지 버전과 현재 프로젝트 버전이 다르면 경고를 보여줘요. 검사 대상은 스니펫 상단의 `@requires`에 적힌 패키지예요. (예: `@seed-design/react`, `@seed-design/css`, `ui:app-screen`의 `@stackflow/react`)
...
현재 프로젝트의 패키지 버전과 스니펫 요구 버전이 호환되는지 검사해요. 검사 대상은 각 스니펫 상단의 `@requires`에 적힌 패키지예요.
