...
`@seed-design/css/vars/component`에서는 `typography`만 제공해요. 다른 컴포넌트 vars는 recipe 구현을 위해 생성되는 내부 값이라 3.0부터 export하지 않아요.

<Callout type="warn">

2.x에서는 `typography`를 제외한 `@seed-design/css/vars/component/*` 경로도 import할 수 있었지만 [SemVer 보장 대상이 아니었어요](/react/updates/upgrade#버저닝-정책). 라이브러리가 이 경로를 쓰고 있다면 SEED 3.0으로 올리기 전에 디자인 토큰으로 옮겨야 해요.

</Callout>

필요한 스타일은 공개 컴포넌트와 recipe 클래스를 쓰거나, 값이 직접 필요하면 [디자인 토큰](/foundations/color)(`@seed-design/css/vars`)에서 찾아 쓰세요. 토큰은 **이름도 값도 major에서만 바뀌어요**. 이름이 바뀌거나 사라지는 것은 물론이고, 디자인 판단에 따라 색이 달라지는 것도 major에서만 일어나요. 라이브러리는 자기 소비자에게 버전 약속을 다시 전달해야 하는 입장이라, 이 보장이 특히 중요해요. 자세한 규칙은 [버저닝 정책](/react/updates/upgrade#버저닝-정책)에 있어요.
...
4. typography를 제외한 @seed-design/css/vars/component/* import가 있는지 찾아줘.
   SEED 3.0에서 제거된 경로이니, 같은 값을 디자인 토큰(@seed-design/css/vars)에서 찾아 대체안을 제시해줘.
