...
테마는 HTML 요소의 `data-ride-color-mode`와 `data-ride-user-color-scheme` 속성을 통해 제어됩니다.
...
```html
<!-- 시스템 설정 따르기 -->
<html data-ride-color-mode="system" />

<!-- 라이트 모드로 고정 -->
<html data-ride-color-mode="light-only" />

<!-- 다크 모드로 고정 -->
<html data-ride-color-mode="dark-only" />
```
...
이 값은 `data-ride-color-mode="system"`일 때만 의미를 가지며, `prefers-color-scheme` 미디어 쿼리를 통해 자동으로 업데이트됩니다.

```html
<!-- 시스템 설정을 따르도록 설정했으며, 현재 사용자는 라이트 모드를 사용 중 -->
<html data-ride-color-mode="system" data-ride-user-color-scheme="light"></html>
```
...
/* 다크 모드가 적용되는 경우 */
:root[data-ride-color-mode="system"][data-ride-user-color-scheme="dark"],
...
특정 영역이나 컴포넌트에만 다른 테마를 적용하고 싶을 때는 해당 요소에 `data-ride-color-mode`에 `light-only`, `dark-only` 또는 `system` 값을 설정합니다.
...
이런 경우 `data-ride-color-mode`와 `data-ride-user-color-scheme` 속성을 감지하여 사용할 수 있습니다.
...
- `MutationObserver`를 사용하여 `data-ride-color-mode`와 `data-ride-user-color-scheme` 속성의 변경을 감지합니다.
