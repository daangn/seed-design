...
비활성화된(`disabled`) 버튼은 hover/focus 이벤트를 발생시키지 않아 Help Bubble Tooltip이 열리지 않습니다. 비활성 상태에서도 Help Bubble Tooltip을 보여주려면 버튼을 포커스 가능한 `span`(예: `tabIndex={0}`)으로 감싸 `children`으로 사용하세요.
