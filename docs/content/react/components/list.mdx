...
`borderRadius` prop으로 active 상태에서 적용되는 리스트 항목의 모서리 둥글기를 조정할 수 있습니다. 카드 등 모서리가 둥근 컨테이너 안에 리스트를 넣는 경우 유용합니다.
...
리스트 항목 전체 영역을 `label`로 만들고, `List.Prefix` 또는 `List.Suffix`에 [Swimarkark](/react/components/switch#using-switchMark), [Checkmark](/react/components/checkbox#using-checkmark) 또는 [RadioMark](/react/components/radio-group#using-radiomark)를 넣는 경우 활용할 수 있는 패턴입니다.
