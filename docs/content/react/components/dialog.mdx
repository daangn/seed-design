...
`<DialogContent>`에 `width`, `maxWidth` prop을 전달하여 너비를 직접 제어할 수 있습니다.
...
<ComponentExample name="react/dialog/custom-size">
  ```json doc-gen:file
  {
    "file": "examples/react/dialog/custom-size.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
...
<ComponentExample name="react/dialog/overflow">
  ```json doc-gen:file
  {
    "file": "examples/react/dialog/overflow.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
...
`DialogAction`의 `onClick`에서 `e.preventDefault()`를 호출하면 Dialog가 닫히지 않습니다.
...
- `"cascadeDismiss"`: 상위 레이어가 닫힐 때 함께 닫힘
...
<Callout>
  아래 예시는 `closeOnInteractOutside={true}`를 설정하여 외부 영역 클릭에
  닫히도록 설정되어 있습니다.
</Callout>
