...
`renderDateCellContent`로 날짜 숫자 아래에 가격, 예약 가능 여부, 혼잡도 같은 정보를 표시할 수 있습니다. 함수에는 날짜와 포맷된 날짜 숫자, 선택·오늘·범위·사용 불가·포커스 상태가 전달됩니다.

컴포넌트는 날짜 셀의 DOM, ARIA, 이벤트, 상태 배경을 계속 소유합니다. 따라서 함수에서는 버튼이나 `gridcell`을 만들지 말고 콘텐츠만 반환합니다. 각 주의 셀 높이는 가장 높은 콘텐츠에 맞춰 늘어납니다. 좁은 화면에서 콘텐츠가 가로로 넘치는 경우에는 렌더 함수에서 `maxLines` 같은 말줄임 정책을 직접 적용합니다.
...
`visibleRange`는 다음 네 가지 레이아웃을 제공합니다.

- `month`: 현재 월의 실제 주 수만 렌더링합니다.
- `twoMonths`: 연속한 두 달을 가로로 표시합니다. 이전·다음 버튼은 한 달씩 이동합니다.
- `week`: 한 주만 표시하며 이전·다음 버튼은 한 주씩 이동합니다.
- `continuous`: `yearRange` 안의 월을 세로로 가상화합니다. 스크롤 영역이 생기도록 `height` 또는 `maxHeight`를 반드시 지정합니다.

Month와 Two Months는 항상 6주 높이를 확보하지 않고 각 월에 필요한 4~6주만 렌더링합니다.

Date Picker는 자체적으로 고정 폭이나 최대 폭을 제한하지 않고 부모 컨테이너의 너비를 채웁니다. Month와 Two Months에 필요한 가로 크기는 사용하는 화면에서 적절한 너비의 컨테이너로 감싸서 결정하세요.
...
`viewDate`, `defaultViewDate`, `onViewDateChange`로 표시 기준 날짜를 제어할 수 있습니다. Month 계열은 월의 첫날, Week는 locale의 주 시작일로 정규화합니다.
...
이전·다음 버튼과 연도·월 휠의 기본 접근성 이름은 한국어와 영어로 제공됩니다. 제품 맥락에 맞는 이름이 필요하면 `ariaLabels`로 재정의합니다.
