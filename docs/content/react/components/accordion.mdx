...
단일 확장 모드에서 `collapsible` prop을 사용하여 열린 항목을 다시 클릭했을 때 닫힘 여부를 제어할 수 있습니다. 기본값은 `true`입니다. `collapsible={false}`로 설정하면 항상 하나의 항목이 열려 있어야 합니다. `multiple` 모드에서는 각 항목이 독립적으로 열리고 닫히므로 `collapsible`을 사용할 수 없습니다.
...
`multiple` controlled 모드에서는 현재 열려 있는 항목들이 `values` 배열로 전달됩니다. 아래 예시는 트리거를 누를 때마다 최신 `values`와 최근 `onValuesChange` 결과를 함께 보여줍니다.
