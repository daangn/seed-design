...
Accordion prefix icon은 List의 prefix icon처럼 텍스트 옆에서 항목 의미를 보조하므로 기본적으로 Line icon을 권장합니다. `size`나 `variant`와 관계없이 22px 기준을 유지하며, Fill icon은 16px 이하의 작은 아이콘, 버튼/칩처럼 컨테이너 안의 아이콘, ON/OFF 상태 표현에 더 적합합니다. 자세한 기준은 [Iconography Usage](/docs/foundation/iconography/usage)를 참고하세요.
