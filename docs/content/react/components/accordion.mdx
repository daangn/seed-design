...
<react-type-table
  path="./registry/react/ui/accordion.tsx"
  name="AccordionProps"
/>
...
<ComponentExample name="react/accordion/multiple">
  ```json doc-gen:file
  {
    "file": "examples/react/accordion/multiple.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
...
<ComponentExample name="react/accordion/controlled-required-open">
  ```json doc-gen:file
  {
    "file": "examples/react/accordion/controlled-required-open.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
...
`size` prop으로 아코디언의 크기를 설정합니다. 기본값은 `medium`(모바일 기본 사이즈)이고, `large`는 PC 대응 사이즈입니다. `responsive`는 모바일에서 Medium, `md`(768px) 이상에서 Large로 자동 전환됩니다.

<ComponentExample name="react/accordion/size">
  ```json doc-gen:file
  {
    "file": "examples/react/accordion/size.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
