---
title: Menu Sheet
description: 사용자의 작업과 관련된 선택지를 제공하는 시트 형태의 컴포넌트입니다.
---

<ComponentExample name="react/menu-sheet/preview">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/preview.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

<Card title="Stackflow" href="/react/stackflow/menu-sheet" variant="example">

Stackflow와 Menu Sheet를 함께 사용하는 방법에 대해 알아보세요.

</Card>

## Installation

```package-install
npx @seed-design/cli@latest add ui:menu-sheet
```

<ManualInstallation name="menu-sheet" />

## Props

### `MenuSheetRoot`

<react-type-table
  path="./registry/ui/menu-sheet.tsx"
  name="MenuSheetRootProps"
/>

### `MenuSheetTrigger`

<react-type-table
  path="./registry/ui/menu-sheet.tsx"
  name="MenuSheetTriggerProps"
/>

### `MenuSheetContent`

<react-type-table
  path="./registry/ui/menu-sheet.tsx"
  name="MenuSheetContentProps"
/>

### `MenuSheetGroup`

<react-type-table
  path="./registry/ui/menu-sheet.tsx"
  name="MenuSheetGroupProps"
/>

### `MenuSheetItem`

<react-type-table
  path="./registry/ui/menu-sheet.tsx"
  name="MenuSheetItemProps"
/>

## Examples

### Trigger

`<MenuSheetTrigger>`는 `asChild` 패턴을 사용해 자식 요소가 MenuSheet를 열 수 있도록 합니다.

`<MenuSheetTrigger>`는 `aria-haspopup="dialog"` 속성을 설정하고, `MenuSheet`의 `open` 상태에 따라 `aria-expanded` 속성을 자동으로 설정합니다. 이 속성은 스크린 리더와 같은 보조 기술에 유용합니다.

<ComponentExample name="react/menu-sheet/trigger">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/trigger.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Controlled

Trigger 외의 방식으로 MenuSheet를 열고 닫을 수 있습니다. 이 경우 `open` prop을 사용하여 MenuSheet의 상태를 제어합니다.

<ComponentExample name="react/menu-sheet/controlled">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/controlled.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### `onOpenChange` Details

`onOpenChange` 두 번째 인자로 `details`가 제공됩니다.

#### `reason`

**닫힐 때** (`open: false`)

- `"closeButton"`: `MenuSheet.CloseButton`으로 닫힘
- `"escapeKeyDown"`: <kbd>ESC</kbd> 키 사용
- `"interactOutside"`: 외부 영역 클릭
- `"drag"`: 드래그로 닫힘
- `"handleClickOnLastSnapPoint"`: 마지막 스냅 포인트에서 핸들 클릭으로 닫힘

<ComponentExample name="react/menu-sheet/open-change-reason">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/open-change-reason.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### With Title

`MenuSheetContent`의 `title` prop을 사용하여 시트 헤더에 제목을 표시합니다.

<ComponentExample name="react/menu-sheet/with-title">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/with-title.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### With Title and Description

`MenuSheetContent`의 `description` prop을 사용하여 제목 아래에 부가 설명을 추가합니다.

<ComponentExample name="react/menu-sheet/with-title-and-description">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/with-title-and-description.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Label Align

`MenuSheetContent`의 `labelAlign` prop으로 메뉴 항목의 레이블 정렬을 설정합니다.

#### `labelAlign="left"` (with `PrefixIcon`)

레이블을 왼쪽 정렬합니다.

<ComponentExample name="react/menu-sheet/with-prefix-icon">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/with-prefix-icon.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

#### `labelAlign="center"` (without `PrefixIcon`)

레이블을 중앙 정렬합니다. (일반적으로, `prefixIcon` 없는 경우)

<ComponentExample name="react/menu-sheet/without-prefix-icon">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/without-prefix-icon.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

#### Overriding `labelAlign`

필요한 경우 `MenuSheetContent`에 지정한 `labelAlign`을 `MenuSheetGroup` 또는 `MenuSheetItem`에 지정한 `labelAlign`으로 덮어쓸 수 있습니다.

<ComponentExample name="react/menu-sheet/overriding-label-align">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/overriding-label-align.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Show Handle

`<MenuSheetContent>`에 `showHandle` prop을 전달하여 Handle을 표시할 수 있습니다.
기본 값은 `false`입니다.

<ComponentExample name="react/menu-sheet/show-handle">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/show-handle.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Show Close Button

`<MenuSheetContent>`에 `showCloseButton` prop을 전달하여 닫기 버튼을 표시할 수 있습니다.
기본 값은 `true`입니다.

showCloseButton을 `false`로 설정하면 닫기 버튼이 표시되지 않습니다.
이 경우 유저가 MenuSheet를 닫을 수 있는 방법을 제공해야 합니다.

<ComponentExample name="react/menu-sheet/show-close-button">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/show-close-button.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Dismissible

`dismissible` prop을 `false`로 설정하면 closeOnEscape, closeOnInteractOutside, draggable 기능이 비활성화됩니다.
의도적으로 MenuSheet를 닫을 수 없게 하고 싶을 때 사용합니다. 이외에는 유저가 MenuSheet를 닫을 수 있는 방법을 제공해야 합니다.

<ComponentExample name="react/menu-sheet/dismissible">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/dismissible.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

### Skip Animation

`skipAnimation` prop을 사용하여 MenuSheet의 enter/exit 애니메이션을 건너뛸 수 있습니다.

<ComponentExample name="react/menu-sheet/skip-animation">
  ```json doc-gen:file
  {
    "file": "examples/react/menu-sheet/skip-animation.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>
