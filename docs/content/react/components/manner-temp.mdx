---
title: Manner Temp
description: 사용자의 매너온도를 시각적으로 표현하는 컴포넌트입니다. 신뢰도/매너 정도를 직관적으로 보여주는 데에 사용합니다.
whenToRead: React에서 매너온도를 MannerTemp로 표시하며 temperature로 레벨을 계산하거나 level을 직접 지정할 때 읽는다.
---

<AvailableSince packages="@seed-design/react@0.0.9, @seed-design/css@0.0.9" />

<ComponentExample name="react/manner-temp/preview">
  ```json doc-gen:file
  {
    "file": "examples/react/manner-temp/preview.tsx",
    "codeblock": true
  }
  ```
</ComponentExample>

## Installation

```package-install
npx @seed-design/cli@latest add ui:manner-temp
```

<ManualInstallation name="manner-temp" />

## Props

<react-type-table path="./registry/react/ui/manner-temp.tsx" name="MannerTempProps" />
