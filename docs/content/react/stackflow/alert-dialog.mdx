...
1. `layerIndex={useZIndexBase()}`로 Alert Dialog Activity의 z-index 기준점을 전달합니다.
    - [`useZIndexBase`에 대해 자세히 알아보기](#about-usezindexbase)

## Syncing Alert Dialog State with a Step

Alert Dialog를 Activity로 만들 수 없는 경우, Alert Dialog가 표시된 상태를 [Step](https://stackflow.so/docs/get-started/navigating-step)과 동기화할 수 있습니다.
...
- 이는 Step 패턴에서 현재 Activity 위에 AlertDialog를 표시할 때 유용합니다.
  ```
  ┌─ 1번 AppScreen Activity에서 modifier: +1로 띄운 AlertDialog (예: Step 패턴)
