...
1. `layerIndex={useZIndexBase()}`로 Menu Sheet Activity의 z-index 기준점을 전달합니다.
    - [`useZIndexBase`에 대해 자세히 알아보기](#about-usezindexbase)

## Syncing Menu Sheet State with a Step

Menu Sheet를 Activity로 만들 수 없는 경우, Menu Sheet가 표시된 상태를 [Step](https://stackflow.so/docs/get-started/navigating-step)과 동기화할 수 있습니다.
...
`useZIndexBase`는 각 Activity의 z-index 기준점을 반환하는 훅입니다..
...
- 두 훅이 반환하는 값은 같으나, `@stackflow/plugin-basic-ui`가 아닌, SEED 컴포넌트를 사용하는 경우 SEED가 관리하는 `@seed-design/stackflow` 패키지의 `useZIndexBase` 훅을 사용하는 것을 권장합니다.
