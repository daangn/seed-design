...
whenToRead: Stackflow 앱에서 Bottom Sheet를 Activity로 만들거나 Step과 동기화할 때, 위에 Activity를 push해도 시트와 폼 상태를 유지하거나 Alert Dialog 같은 modal을 겹쳐 올릴 때 읽는다.
---
...
1. `modal={isActive}`로 Bottom Sheet Activity가 비활성 상태일 때 `modal`을 `false`로 설정합니다. 이렇게 하지 않으면, 위에 push된 Activity에서 포커스, 스크린 리더 접근 및 스크롤 등의 상호작용이 동작하지 않습니다.
