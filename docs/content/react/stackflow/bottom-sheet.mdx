...
Bottom Sheet를 Activity로 만들 수 없는 경우, Bottom Sheet가 표시된 상태를 [Step](https://stackflow.so/docs/get-started/navigating-step)과 동기화할 수 있습니다.
...
`BottomSheetContent` 컴포넌트가 Activity 위
의 `layerIndex` prop에 전달하는 `useZIndexBase` 훅에 대한 자주 묻는 질문입니다.
