...
이미지, 가로 스크롤 영역, 배경 띠처럼 화면 끝까지 채워야 하는 요소는 `Box`의 `bleedX`에 `"safeArea"`를 지정해 좌우 inset만큼 넓힙니다. 넓힌 요소 안의 내용을 safe area 안쪽에 두려면 `pl`, `pr`에도 `"safeArea"`를 함께 지정합니다.

가로 스크롤 영역은 flex 컨테이너(`display="flex"`)로 두세요. WebKit은 block 스크롤 컨테이너의 끝쪽 padding을 스크롤 범위에 넣지 않아서, 끝까지 스크롤하면 마지막 항목이 오른쪽 inset 영역에 걸립니다.
