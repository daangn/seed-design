...
safe area 처리에 필요한 viewport 설정은 [Safe Area 구현 가이드](/react/components/concepts/safe-area#viewport-설정)를 참고하세요.
...
좌우 inset이 반영되므로 `AppScreenContent` 안의 요소는 화면 끝까지 닿지 않습니다. 이미지, 가로 스크롤 영역, 배경처럼 화면 끝까지 채워야 하는 요소는 inset만큼 다시 넓혀야 합니다. 구현 방법은 [Safe Area 구현 가이드](/react/components/concepts/safe-area#화면-끝까지-채우기)를 참고하세요.
