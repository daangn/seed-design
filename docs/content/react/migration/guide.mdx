---
title: Migrating from Sprout
description: Sprout에서 SEED React로 마이그레이션하기 위해 필요한 작업들을 안내합니다.
---

## 마이그레이션 개요

Sprout에서 SEED React로의 마이그레이션은 아래와 같은 순서로 진행됩니다.
...
<Callout>
  Sprout `@seed-design/stylesheet` 패키지의 `global.css`와 상호운용이
  가능합니다.
</Callout>

<Callout type="warn">
  번들러
  플러그인([webpack](https://github.com/daangn/seed-design/tree/dev/packages/webpack-plugin),
  [vite](https://github.com/daangn/seed-design/tree/dev/packages/vite-plugin),
  [rsbuild](https://github.com/daangn/seed-design/tree/dev/packages/rsbuild-plugin))을
  사용하는 경우에는 `all.css` 대신 `base.css`를 사용해주세요.
</Callout>
...
- `data-seed`: Seed Design 전역 테마 적용 (SEED에서 그대로 유지)
...
4. `@seed-design/stylesheet` 패키지를 사용하는 용례
