---
title: Guide
description: Sprout에서 SEED로 마이그레이션하기 위해 필요한 작업들을 안내합니다.
whenToRead: 마이그레이션 가이드로, Sprout을 쓰던 프로젝트를 SEED로 옮기며 작업 순서를 정하거나, data-seed-scale-color 테마 속성을 바꾸거나, 스타일 방식에 맞는 codemod를 고르거나, issues.log 경고를 해결할 때 읽는다.
---


## 마이그레이션 개요

Sprout에서 SEED로 넘어가기 위해서는 크게 아래와 같은 순서로 진행됩니다.
...
SEED를 사용하기 위한 기반 작업들을 먼저 진행합니다.
아래 작업들이 끝나면 SEED 사용이 가능합니다.
...
<Callout>
Sprout `@seed-design/stylesheet` 패키지의 `global.css`와 상호운용이 가능합니다.
</Callout>
...
SEED 스타일시트 사용법은 [Installation의 css import](/react/getting-started/installation/manual#allcss-import하기) 설명을 참고해주세요.
...
기존 Sprout에서 사용되던 data-attribute가 바뀌었습니다.
...
- `data-seed`: SEED 전역 테마 적용 (SEED에서 그대로 유지)
- `data-seed-color-mode`: SEED 테마 강제 적용
  - `light-only`, `dark-only`, `system` 중 하나를 선택할 수 있습니다.
- `data-seed-user-color-scheme`: 유저 기기의 설정
  - `light`, `dark` 중 하나의 값을 가집니다.
  - Sprout에서 `data-seed-scale-color` 속성을 사용하던 코드가 있다면 `data-seed-user-color-scheme` 속성으로 변경해주세요.
...
- 위 컴포넌트를 사용중인 코드 용례
- `@seed-design/stylesheet` 패키지를 사용하는 용례
...
- 전처리해서 사용하는 경우에는 전처리 코드 자체를 SEED로 마이그레이션 한 후 나머지 코드들을 Codemod로 마이그레이션 하는게 좋습니다.
...
### Sprout-SEED Compatibility

Color, Typography 마이그레이션이 끝나고 나면 기존 Sprout 컴포넌트와의 호환성 문제가 있을 수 있습니다.

Sprout 내부에 Sprout 토큰을 SEED 토큰으로 전부 교체한 Compatibility 버전을 제공합니다.
...
Sprout-SEED Compatibility 버전까지 설치가 되었다면 파운데이션 마이그레이션이 끝난 것으로 간주할 수 있습니다.
이론상 이때부터는 `@seed-design/design-token` 패키지와 `@seed-design/stylesheet` 패키지를 제거할 수 있습니다.
또한 SEED 컴포넌트를 점진적 사용이 가능합니다.
...
기존에 25.10.0 앱버전 이상에서 Sprout 백그라운드 색상을 SEED로 변경하기 위해 추가했던 스크립트는
...
- SEED에서 지원하지 않는 토큰을 사용하는 경우입니다. (주로 alpha 색상 토큰이거나 너무 작거나 큰 타이포 토큰입니다.)
...
- SEED에서 static 컬러 토큰은 웬만해서는 `white`, `black` 두 가지만 지원합니다.
...
- 하지만 SEED에서 Deprecated 된 토큰인 경우, 대체 토큰으로 매핑 될 수 있습니다.
...
### Vanilla Extract CSS + Vite + SEED 사용시
