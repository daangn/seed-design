...
```package-install
@ride-developer/react @ride-developer/css
```

- `@ride-developer/react`는 리액트 컴포넌트 라이브러리가 전부 포함되어 있습니다.
  - 기존 `sprout` 패키지들을 대체합니다.
  - 컴포넌트 마이그레이션 전까지는 기존 패키지들을 사용해주세요.
- `@ride-developer/css`는 스타일 관련된 모든 것이 포함되어 있습니다. (vars, theming, recipe, css, etc.)
  - 기존 `@ride-developer/stylesheet` 라이브러리를 대체합니다.
  - 기존 `@ride-developer/design-token` 라이브러리를 대체합니다.
...
<Callout>
V2 `@ride-developer/stylesheet` 패키지의 `global.css`와 상호운용이 가능합니다.
</Callout>

<Callout type="warn">
번들러 플러그인([webpack](https://github.com/daangn/seed-design/tree/dev/packages/webpack-plugin), [vite](https://github.com/daangn/seed-design/tree/dev/packages/vite-plugin), [rsbuild](https://github.com/daangn/seed-design/tree/dev/packages/rsbuild-plugin))을 사용하는 경우에는 `all.css` 대신 `base.css`를 사용해주세요.
</Callout>

```ts title="index.tsx"
import "@ride-developer/css/all.css"; // [!code ++]
```
...
```html
<!-- [!code --] -->
<html data-ride data-ride-scale-color="light" />
<!-- [!code ++] -->
<html data-ride data-ride-color-mode="system" data-ride-user-color-scheme="light" />
```

- `data-ride`: Seed Design 전역 테마 적용 (V3에서 그대로 유지)
- `data-ride-color-mode`: Seed Design 테마 강제 적용
  - `light-only`, `dark-only`, `system` 중 하나를 선택할 수 있습니다.
- `data-ride-user-color-scheme`: 유저 기기의 설정
  - `light`, `dark` 중 하나의 값을 가집니다.
  - v2에서 `data-ride-scale-color` 속성을 사용하던 코드가 있다면 `data-ride-user-color-scheme` 속성으로 변경해주세요.
...
- [@ride-developer/vite-plugin](https://github.com/daangn/seed-design/tree/dev/packages/vite-plugin)
...
- [@ride-developer/rsbuild-plugin](https://github.com/daangn/seed-design/tree/dev/packages/rsbuild-plugin)
...
- [@ride-developer/webpack-plugin](https://github.com/daangn/seed-design/tree/dev/packages/webpack-plugin)
...
`@ride-developer/css/theming` 패키지에서 제공하는 `generateThemingScript` 함수를 사용해서 스크립트를 생성해요.

```ts title="index.tsx"
import { generateThemingScript } from "@ride-developer/css/theming";
```
...
우선 현재 프로젝트에서 `@ride-developer/design-token` 패키지를 사용하는 용례를 분석해야합니다.

- `@ride-developer/design-token` 패키지의 vars를 사용하는 용례
- `@ride-developer/design-token` 패키지의 vars를 확장해서 사용하는 컴포넌트
- 위 컴포넌트를 사용중인 코드 용례
- `@ride-developer/stylesheet` 패키지를 사용하는 용례
- `--seed-` prefix로 시작하는 CSS Variable을 사용하는 용례
- 사용중인 CSS 라이브러리와 `@ride-developer/design-token` 패키지와 결합해서 사용하는 용례
...
1. `@ride-developer/design-token` 패키지의 vars 변수를 사용하는 용례
2. `@ride-developer/design-token` 패키지의 vars 변수를 확장해서 사용하는 컴포넌트
3. 2번 컴포넌트를 사용중인 코드 용례 
4. `@ride-developer/styleesheet` 패키지를 사용하는 용례
5. `--seed-` prefix로 시작하는 CSS Variable을 사용하는 용례
6. 사용중인 CSS 라이브러리와 `@ride-developer/design-token` 패키지와 결합해서 사용하는 용례
...
- [replace-custom-seed-design-typography](https://seed-design.io/react/developer-tools/codemods/available-transforms#replace-custom-seed-design-typography) 커스텀 transform을 사용해서 마이그레이션 할 수 있습니다.

```ts title="typography.ts"
import type { TextVariant } from '@ride-developer/css/recipes/text';

import { vars as typographyVars } from '@ride-developer/css/vars/component/typography';
...
```package-install
npx @ride-developer/codemod@latest <transform> <target_path> <옵션>
```
...
- [@ride-developer/tailwind3-plugin](https://github.com/daangn/seed-design/tree/dev/packages/tailwind3-plugin)
- [@ride-developer/tailwind4-theme](https://github.com/daangn/seed-design/tree/dev/packages/tailwind4-theme)
...
- Emotion, Vanilla Extract CSS와 같은 CSS-in-JS 라이브러리는 `@ride-developer/design-token`을 사용하는 경우가 대부분입니다.
- [replace-seed-design-token-vars](/react/developer-tools/codemods/available-transforms#replace-seed-design-token-vars) transform을 사용해서 일괄적으로 마이그레이션 할 수 있습니다.
- 해당 transform은 `color`와 `typography` 두 개 동시에 변경합니다.

#### Native CSS

- 기본적으로 `--seed-` prefix가 붙은 CSS Variable을 사용하는 경우가 있습니다.
- [replace-css-seed-design-color-variable](/react/developer-tools/codemods/available-transforms#replace-css-seed-design-color-variable) CSS transform도 제공합니다.
...
이론상 이때부터는 `@ride-developer/design-token` 패키지와 `@ride-developer/stylesheet` 패키지를 제거할 수 있습니다.
...
- `@vanilla-extract/vite-plugin`와 `@ride-developer/vite-plugin`을 같이 사용하는 경우 빌드가 실패할 수 있습니다.
