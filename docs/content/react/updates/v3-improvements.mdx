...
V2에서는 TextField, MultilineTextField 컴포넌트를 따로 제공했지만, V3에서는 조합형 컴포넌트로 통합되었어요. 자세한 사용법은 [TextField](/react/components/text-field) 및 [MultilineTextField](/react/components/multiline-text-field) 문서를 참고해주세요.
...
V2에서는 `@daangn/sprout-components-select-box` 패키지에서 여러 SelectBox 관련 컴포넌트들을 제공했어요. V3에서는 이러한 컴포넌트들이 목적에 따라 재구성되었어요. 자세한 사용법은 [CheckSelectBox](/react/components/check-select-box) 및 [RadioSelectBox](/react/components/radio-select-box) 문서를 참고해주세요.
...
- [`AppScreen`](/react/components/stackflow/app-screen): Stackflow AppScreen 컴포넌트
