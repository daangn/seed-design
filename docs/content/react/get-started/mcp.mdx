...
```typescript
import { defineComponentTransformer, createElement } from "@seed-design/figma";
...
### MCP로 ComponentTransformer 작성하기
...
```markdown
# ComponentTransformer Implementation Guide
...
5. **Create Component Transformer**:
   - Implement the transformer using the following pattern:

\`\`\`typescript
import {
  createElement,
  defineComponentTransformer,
  type InferFromDefinition,
} from "@seed-design/figma";

// Define component properties based on Figma definition
export type ComponentNameProperties = InferFromDefinition<{
...
export const componentNameTransformer = defineComponentTransformer<ComponentNameProperties>(
...
6. **Verify Transformer Functionality**:
   - Test the transformer with different component variants
...
**예시**:
```bash
@CustomButton.tsx @ComponentTransformerRule.md CustomButton 컴포넌트의 ComponentTransformer를 구현해주세요.
```
</Step>
<Step>
#### 3. 결과 확인
프롬프트 실행 결과로 생성된 ComponentTransformer를 확인합니다.
...
export type CustomButtonProperties = InferFromDefinition<{
  tone: {
    type: "VARIANT";
...
export const customButtonTransformer = defineComponentTransformer<CustomButtonProperties>(
  "figma_component_key",
  ({ componentProperties: props }) => {
    const tone = props.tone.value.toLowerCase();
...
생성된 ComponentTransformer를 설정 파일에 추가합니다.

```typescript
import { customButtonTransformer } from "./CustomButton";

export default {
  extend: {
    componentTransformers: [
      customButtonTransformer,
    ]
  },
};
```
