...
`replace-react-icons` transform을 실행해요.

```shell
npx @seed-design/codemod@latest replace-react-icons <target_path>
```

<Card title="replace-react-icons" href="/react/developer-tools/codemods/available-transforms#replace-react-icons" icon={<Terminal/>}>
