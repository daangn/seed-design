...
**Figma URL만으로 레이어 정보를 가져옵니다.**

[Figma 설정](https://www.figma.com/settings)에서 Personal Access Token을 발급받고 `FIGMA_PERSONAL_ACCESS_TOKEN` 환경 변수로 설정합니다.
...
**웹소켓 서버와 Figma 플러그인을 통해 Figma 앱에서 실시간으로 선택한 레이어 정보를 가져옵니다.**

`get_selection` 등 실시간 선택 기반 도구를 사용하려면 이 모드가 필요합니다.

1. [Figma MCP 플러그인](https://www.figma.com/community/plugin/1496384010980477154)을 설치합니다.
2. WebSocket 서버를 실행합니다.

- bun 런타임이 필요합니다. bun과 --bun flag를 사용해주세요.

```bash
bunx --bun @seed-design/mcp@latest socket
```

3. Figma에서 플러그인을 실행하고 연결합니다.
...
Figma에서 레이어의 URL을 복사해 LLM 도구에게 요청합니다.
...
Figma에서 레이어를 선택한 후 LLM 도구에게 요청합니다.
...
Figma URL 또는 `fileKey` + `nodeId`를 전달하면 REST API로, `nodeId`만 전달하면 WebSocket으로 동작합니다.
...
SEED Figma Slider 컴포넌트를 SEED React `<Slider />`로 매핑하는 핸들러 예시를 [여기](https://github.com/daangn/seed-design/blob/dev/packages/figma/src/codegen/targets/react/component/handlers/slider.ts)에서 확인할 수 있습니다.

</Callout>

`--config` 플래그를 통해 커스텀 설정 파일을 로드합니다.
