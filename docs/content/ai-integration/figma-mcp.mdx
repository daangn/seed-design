...
2. MCP가 환경 변수를 통해 해당 토큰을 사용할 수 있도록 합니다.
...
WebSocket을 사용하는 경우 `FIGMA_PERSONAL_ACCESS_TOKEN` 환경 변수는 설정하지 않아도 됩니다.
...
MCP 서버 설정에 `--config` 플래그를 추가해 커스텀 설정 파일이 로드될 수 있도록 합니다.

현재 `--config` 플래그는 Bun 런타임이 필요합니다. `--bun` flag를 사용해주세요.
