...
2. [Figma MCP 플러그인](https://www.figma.com/community/plugin/1496384010980477154)을 설치하고 실행합니다.
...
MCP 서버 실행 시 `--mode` 옵션을 명시하여 해당 모드에서 사용 가능한 도구만 등록할 수 있습니다.

| 모드 | 설명 | 비고 |
| ----------- | ----------------------- | |
| `all` | 모든 도구 등록 (기본값) | Personal Access Token이 제공된 경우 REST API를 사용하면서, WebSocket 전용 도구를 호출하는 경우 WebSocket 서버에 연결을 시도합니다. |
| `rest` | REST API 도구만 등록 | |
| `websocket` | WebSocket으로 접근 가능한 도구만 등록 | WebSocket 모드에서는 모든 도구를 사용 가능합니다. |
...
```json title=".mcp.json"
{
  "mcpServers": {
    "seed-design-figma": {
      "command": "bunx",
      "args": ["-y", "@seed-design/mcp@latest", "--mode", "rest"],
      "env": {
...
```json title=".mcp.json"
{
  "mcpServers": {
    "seed-design-figma": {
      "command": "bunx",
      "args": ["-y", "@seed-design/mcp@latest", "--mode", "websocket"]
    }
  }
}
```
