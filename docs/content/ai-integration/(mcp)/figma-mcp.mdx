...
claude mcp add seed-design-figma \
  ...
  -- bunx -y @ride-developer/mcp@latest
```
...
```json
{
  "mcpServers": {
    "seed-design-figma": {
      "command": "bunx",
      "args": ["-y", "@ride-developer/mcp@latest"],
      "env": {
...
```bash
bunx --bun @ride-developer/mcp@latest --config /path/to/config.ts
```

### 지원하는 설정 파일 형식

`@ride-developer/mcp`는 다음 확장자의 설정 파일을 지원합니다:
...
```typescript
import { type react } from "@ride-developer/figma";
