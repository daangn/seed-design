...
도구는 문서 사이트의 현재 문서 목록을 읽으므로, 문서가 추가되거나 옮겨져도 서버를 업데이트할 필요가 없습니다.

### 검색

#### `search_docs`

문서 본문을 검색해 관련 문서를 관련도 순으로 반환합니다. 어느 문서에 답이 있는지 모를 때 사용합니다.
...
문서 목록을 반환합니다. 섹션을 지정하지 않으면 모든 문서를 반환합니다.

**파라미터:**
| 이름 | 타입 | 설명 |
|------|------|------|
| `section` | string (선택) | 섹션 이름 (예: `react`, `components`, `foundations`) |
...
| 이름 | 타입 | 설명 |
|------|------|------|
| `path` | string | `search_docs`나 `list_docs`가 출력한 문서 주소 |
