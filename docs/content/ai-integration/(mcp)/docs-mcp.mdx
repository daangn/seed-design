...
문서 목록을 `search_docs`와 같은 형식으로 반환합니다. 한 줄에 문서 하나씩, 주소 뒤에 제목과 설명이 붙습니다. 섹션을 지정하지 않으면 모든 문서를 반환합니다.

**파라미터:**
| 이름 | 타입 | 설명 |
|------|------|------|
| `section` | string (선택) | 섹션 이름. 문서 주소의 첫 경로를 슬래시 없이 씁니다 (예: `react`, `components`, `foundations`) |
...
| 이름 | 타입 | 설명 |
|------|------|------|
| `path` | string | 문서 주소. `search_docs`나 `list_docs`가 출력한 주소를 슬래시까지 그대로 씁니다. 뒤에 붙은 `#앵커`는 무시하고 문서 전체를 반환합니다 |
...
| 이름 | 타입 | 설명 |
|------|------|------|
| `path` | string (선택) | 리소스 경로. 리소스 목록에 나온 경로를 슬래시까지 그대로 씁니다 (예: `/color.json`, `/components/action-button.json`) |
