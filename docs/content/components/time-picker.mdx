...
Time Picker는 선택 상태를 관리하는 root 아래에, 휠 UI를 담는 surface를 둡니다. Layout이 Popover·Sheet일 때는 surface를 열고 닫는 trigger가 더해집니다.

- **Trigger:** Layout이 Popover·Sheet일 때만 존재합니다. 선택된 시간 또는 placeholder를 보여주고, 누르면 surface를 엽니다. prefixIcon(시계 아이콘) · valueText · suffixIcon로 구성됩니다.
- **Surface:** 휠 본체입니다. Inline이면 화면 흐름 안에 그대로 자리하고, Popover·Sheet이면 트리거에서 떠오르는 컨테이너가 됩니다.
...
- **Footer:** Popover·Sheet일 때 선택을 확정하는 영역입니다. 선택적인 helperText와 confirmButton을 가집니다. Inline은 footer가 없으며, 확정은 화면의 액션 영역이 담당합니다.

시·분 휠은 시·분 같은 단위 접미사 없이 숫자만 보여주고, 단위는 오전·오후 칼럼과 선택 위치로 전달됩니다(예: 오전 10 00 = 오전 10시 00분).

<FigmaImage
  id="1958:12513"
  alt="Time Picker의 Anatomy 이미지. Trigger, Surface, Period Column, Hour Column, Minute Column, Selection Indicator, Footer로 구성됩니다."
/>
...
휠을 처음 열면 이미 선택된 시간이 있으면 그 값을, 없으면 현재 시각을 Minute Step에 맞춰 반올림한 값을 가운데에 두고 엽니다. 사용자가 현재 시점을 기준으로 빠르게 조정할 수 있습니다.
