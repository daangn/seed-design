---
slug: /components/checkbox/visual
title: Checkbox
description: 최소 1가지 이상의 옵션을 선택 또는 해제할 수 있는 컨트롤입니다.
---

## Anatomy

![checkbox visual anatomy](./checkbox-visual-anatomy.png)

1. Root : 체크박스 컴포넌트를 감싸는 컨테이너 영역 - Primitive.Root
2. Control : 체크박스의 상태를 나타내는 시각 요소 - Primitive.Control
3. Label : 체크박스에 대한 정보를 전달하는 라벨 - Primitive.Label
4. Icon: 체크박스의 상태를 표현하는 아이콘

### Visually Representated States

| State    | Condition                                             |
| -------- | ----------------------------------------------------- |
| enabled  | !isDisabled && !isHovered && !isFocused && !isPressed |
| disabled | isDisabled                                            |
| hovered  | !isDisabled && !isHovered && !isFocused               |
| focused  | !isDisabled && !isHovered && !isFocused               |
| pressed  | !isDisabled && !isHovered                             |

| State         | Condition                                    |
| ------------- | -------------------------------------------- |
| unselected    | isIndeterminate === false && value === false |
| selected      | isIndeterminate === false && value === true  |
| indeterminate | isIndeterminate === true                     |

### 디자인 결정 (Color)

| State    | Part                 | Attribute     | Value                           |
| -------- | -------------------- | ------------- | ------------------------------- |
| Enabled  | Control (unselected) | Color         | None                            |
|          |                      | Outline Color | $scale.color.gray-300           |
|          | Control (selected)   | Color         | $semantic.color.primary         |
|          |                      | Outline Color | None                            |
|          | Label                | Color         | $semantic.color.ink-text        |
|          | Icon (selected)      | Color         | $semantic.color.on-primary      |
| Disabled | Control (unselected) | Color         | $scale.color.gray-200           |
|          |                      | Outline Color | $scale.color.gray-300           |
|          | Control (selected)   | Color         | $scale.color.gray-300           |
|          |                      | Outline Color | None                            |
|          | Label                | Color         | $scale.color.gray-300           |
|          | Icon (selected)      | Color         | $scale.color.gray-500           |
| Hovered  | Control (unselected) | Color         | $semantic.color.gray-hover      |
|          |                      | Outline Color | $scale.color.gray-300           |
|          | Control (selected)   | Color         | $semantic.color.primary-hover   |
|          |                      | Outline Color | None                            |
|          | Label                | Color         | $semantic.color.ink-text        |
|          | Icon (selected)      | Color         | $semantic.color.on-primary      |
| Pressed  | Control (unselected) | Color         | $semantic.color.pray-pressed    |
|          |                      | Outline Color | $scale.color.gray-300           |
|          | Control (selected)   | Color         | $semantic.color.primary-pressed |
|          |                      | Outline Color | None                            |
|          | Label                | Color         | $semantic.color.ink-text        |
|          | Icon (selected)      | Color         | $semantic.color.on-primary      |

### 디자인 결정 (Typography)

| Part  | Attribute      | Value                            |
| ----- | -------------- | -------------------------------- |
| Label | Size           | $scale.dimension.font-size-200   |
|       | Line Height    | $static.line-height.static-small |
|       | Weight         | $static.font-weight.regular      |
|       | Letter Spacing | $scale.letter-spacing.none       |

### 디자인 결정 (Layout)

| Part           | Attribute  | Value |
| -------------- | ---------- | ----- |
| Root           | Min Width  | 130px |
|                | Min Height | 28px  |
| Control        | Size       | 22px  |
| Label          | Text Align | Start |
| Icon           | Size       | 10px  |
| Control / Icon | Padding    | 12px  |

### Overflow

<FullCard>
  <FullCardImageCell>
    ![checkbox-visual-overflow](./checkbox-visual-overflow.png)
  </FullCardImageCell>
  <FullCardDescription>
    label이 2줄을 초과할 경우 텍스트 시작점을 기준으로 위와 같이 개행처리됩니다
  </FullCardDescription>
</FullCard>
