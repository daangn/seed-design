...
whenToRead: 날짜 선택 UI의 Single·Range·Multiple 선택 방식, Month·Two Months·Continuous·Week 표시 범위, Inline·Sheet·Popover 레이아웃, Read Only·Disabled 날짜 처리 같은 디자인 기준이 필요할 때 읽는다.
keywords: ["데이트 피커", "달력", "캘린더", "calendar"]
...
이미 확정되어 바꿀 수 없는 값은 Read Only로 표시합니다. 채움 배경은 유지하되 톤을 낮추고, 눌러도 반응하지 않습니다. Selected·Range Start·Range End에 적용할 수 있습니다.
...
Range에 최소·최대 기간 제약이 있으면 [Callout](/components/callout)으로 미리 안내합니다. 사용자가 선택을 마친 뒤 에러를 마주하기 전에 제약을 알 수 있게 돕습니다. 시작일만 찍힌 미완성 범위에서는 confirmButton을 활성화하지 않습니다.
...
<FigmaImage
  id="2455:4292"
  alt="Read Only로 고정된 시작일과 Disabled 날짜를 대비해 보여주는 광고 기간 연장 예시"
/>

수정할 수 없는 날짜는 두 가지로 구분합니다. 이미 확정된 값은 Read Only로, 고를 수 없는 날짜는 Disabled로 표시합니다. Read Only는 흐리게 표시하되 채움 배경을 남겨 값이 있다는 것을 알 수 있게 하고, Disabled는 배경 없이 흐리게만 표시합니다.

진행 중인 광고 기간을 연장하는 경우가 대표적입니다. 시작일만 Read Only로 고정하고, 종료일은 자유롭게 선택합니다. 캘린더 전체가 보기 전용인 화면이라면 셀마다 지정하지 않고 캘린더 전체를 Read Only로 둡니다.
