...
수정할 수 없는 날짜는 두 가지로 구분합니다. 이미 확정된 값은 Read Only로, 고를 수 없는 날짜는 Disabled로 표시합니다.

Read Only 값은 흐리게 표시하되 채움 배경을 남겨, 값이 있다는 것을 알 수 있게 합니다. Disabled는 배경 없이 흐리게만 표시합니다.

진행 중인 광고 기간을 연장하는 경우가 대표적입니다. 시작일만 Read Only로 고정하고, 종료일은 자유롭게 선택합니다.

캘린더 전체가 보기 전용인 화면이라면 셀마다 지정하지 않고 캘린더 전체를 Read Only로 둡니다.
