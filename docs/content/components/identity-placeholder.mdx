...
whenToRead: Avatar 이미지가 없을 때 쓸 Person·Business 타입 선택이나, 라이트/다크 분기가 어려운 레거시 환경용 대체 에셋 URL이 필요할 때 읽는다.
keywords: ["아이덴티티 플레이스홀더"]
coverImage: /og/components/identity-placeholder
---
...
Person과 Business 두 가지 타입을 제공합니다. 두 타입 모두 [Avatar](/components/avatar) 컴포넌트에서 사용자가 제공한 이미지가 없거나 표시할 수 없는 상태일 때 대체 이미지로 사용됩니다.
