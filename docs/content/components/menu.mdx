...
whenToRead: 트리거에 붙어 뜨는 액션 목록의 Small·Medium 사이즈, 항목 그룹화와 최대 높이, Destructive 항목, placement, 아이콘·라벨 작성 같은 디자인 기준이 필요할 때 읽는다.
keywords: ["메뉴"]
coverImage: /og/components/menu
---
...
메뉴의 높이는 최대 480px과 Trigger 주변에 남은 화면 공간 중 더 작은 값으로 정해지고, 옵션이 그보다 많으면 메뉴 안에서 스크롤됩니다. 옵션이 많은 Select는 메뉴 안에서 스크롤되는 상태를 가정하고 설계합니다.
