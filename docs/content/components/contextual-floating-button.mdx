...
whenToRead: 콘텐츠 근처에 조건부로 띄우는 버튼의 Solid·Layer tone, Icon Only 레이아웃, 여러 개 동시 배치와 여백 같은 디자인 기준이 필요할 때 읽는다.
keywords: ["플로팅 버튼"]
coverImage: /og/components/contextual-floating-button
---
...
화면의 주 액션을 담당하는 [Floating Action Button](/components/floating-action-button)과 달리, 특정 문맥 또는 사용자 상호작용에 따라 유동적으로 등장합니다.
