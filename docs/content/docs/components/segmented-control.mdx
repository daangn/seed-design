...
선택지가 너무 많아지면 사용자가 인식하기 어렵고, 모바일 환경에서는 터치 영역이 좁아져 사용성이 떨어집니다. 더 많은 옵션이 필요하면 [Radio](/docs/components/radio) group, [Checkbox](/docs/components/checkbox) group와 같은 다른 구성 요소를 사용하는 것이 좋습니다.
...
같은 콘텐츠 목록을 여러 다른 관점이나 기준으로 보여주고 싶을 때, Segmented Control로 각 기준을 손쉽게 전환하도록 만들 수 있습니다.
