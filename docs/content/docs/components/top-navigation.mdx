...
현재 화면을 종료하고 상위 레이어로 복귀합니다. 페이지 히스토리를 거슬러 올라가는 것이 아니라, 열려있는 모달, Bottom Sheet, 전체 화면 등을 닫는 동작입니다.

- 모달/풀스크린 모달/Bottom Sheet처럼 레이어를 닫는 경우
...
액션이 3개를 초과하는 경우, 더보기 버튼과 Menu Sheet를 조합하여 중요한 액션만 노출하고 나머지는 숨기는 것을 권장합니다.
