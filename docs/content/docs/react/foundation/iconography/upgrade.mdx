...
두 패키지는 [`@daangn/react-icon`](https://github.com/daangn/seed-icon-v3/pkgs/npm/react-icon)으로 대체됩니다.
...
문제 없이 모든 변환이 완료된 경우 다음과 같은 결과가 표시됩니다.
...
파싱 오류 등으로 인해 코드 확인이 이루어지지 못한 파일의 수가 `errors`로 표시됩니다. 에러가 있다면, 문제가 발생한 파일을 수정하고 다시 스크립트를 실행하거나, 수동으로 수정해주세요.
...
이전 아이콘이 모두 대체된 경우, 이전 패키지를 제거합니다.
...
[사이드 이펙트](#발생할-수-있는-사이드-이펙트)가 발생했는지 확인하여 의도한 대로 변경이 이루어졌는지 검토합니다.
...
| `icon_write_tory` | [`icon_horizline2_vertical_chatbubble_right`](/docs/design/foundation/iconography?icon=icon_horizline2_vertical_chatbubble_right_fill) |
...
**이전 아이콘과 리뉴얼된 아이콘은 `n:1`로 대응됩니다**. 따라서, 같은 페이지에 표시되었던 서로 다른 두 아이콘이, 동일한 리뉴얼된 아이콘으로 대체되는 경우가 있습니다. 의도한 목적대로 아이콘이 표시되는지 확인합니다.

- 한 아이콘 내의 `thin` variant와 `regular` variant는 모두 `line` variant로 대체됩니다.
  - 예를 들면, `icon_forward_thin`, `icon_forward_regular`는 모두 [`icon_arrow_right_line`](/docs/design/foundation/iconography?icon=icon_arrow_right_line)으로 대체됩니다.
- 완전히 다른 이전 아이콘이 하나의 리뉴얼된 아이콘으로 대체되는 경우도 있습니다.
  - 예를 들면, `icon_headphone_regular`, `icon_helpcenter_regular`, `icon_helper_regular`는 모두 [`icon_headset_line`](/docs/design/foundation/iconography?icon=icon_headset_line)으로 대체됩니다.
