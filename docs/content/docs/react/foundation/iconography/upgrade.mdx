...
아무 문제 없이 실행된 경우 다음과 같은 결과가 표시됩니다.
...
Error 수가 0이 아니면, 파싱 오류 등으로 인해 코드 확인이 이루어지지 못한 파일이 있는 것입니다.
...
변환되지 않은 파일이 있다면, 문제를 수정하고 다시 스크립트를 실행하거나, 수동으로 수정해주세요.
