...
whenToRead: 화면 요소의 쌓임 순서와 깊이를 설계하며 바텀시트·Alert Dialog 같은 Global 레벨과 플로팅 액션·일시적 피드백 같은 Local 레벨을 구분하거나, surface color·shadow·stroke 중 고도 표현 방식을 고르거나, layer-basement와 neutral-weak 중 배경 토큰을 고를 때 읽는다.
coverImage: /og/foundations/elevation
---
...
[Top Navigation](/components/top-navigation)과 [List](/components/list)는 동일한 Level 1이지만, 스크롤 시 List가 자연스럽게 Top Navigation 아래로 들어가도록 배치되어야 합니다.
이런 경우, 콘텐츠가 겹칠 때 [Top Navigation](/components/top-navigation)의 elevation level을 높이지 않습니다. 대신 그림자나 라인(스타일)을 추가하여 '구분감'을 줍니다.
...
<TokenReference regex={/\$color\.bg\.layer-/} />
