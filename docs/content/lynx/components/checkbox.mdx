...
Checkbox는 label을 포함한 Root에서 터치를 받지만 **Checkmark만 Self Scale**합니다. 상위 List Item처럼 이미 축소되는 pressable 안에서는 중복 Scale Feedback을 적용하지 않아야 합니다. [Scale Feedback](/lynx/components/concepts/scale-feedback)을 참고하세요.
...
<LynxComponentExample name="lynx/checkbox/fieldset">
  ```json doc-gen:file
  {
    "file": "examples/lynx/checkbox/fieldset.tsx",
    "codeblock": true
  }
  ```
</LynxComponentExample>
...
| 개별 `Checkbox`의 `name` / `value` / `required` / `invalid` | form field props | Lynx에 native form 제출 모델 없음 |
