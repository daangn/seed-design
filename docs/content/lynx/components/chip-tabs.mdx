...
`ChipTabsList`는 자체 native 가로 `scroll-view`를 렌더링하므로 `ScrollFog`로 감싸지 않고 `fading-edge-length`를 직접 전달합니다. `ScrollFog`와 같은 native fading edge가 스크롤할 콘텐츠가 남은 좌우 edge에 표시됩니다.

아래 예제는 최대 너비 360px 안에 15개 라벨을 표시하고 20px 크기의 fog를 적용합니다. List의 내부 좌우 padding 16px은 그대로 유지합니다.
