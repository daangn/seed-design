...
`ScrollFog`는 Registry snippet이나 Recipe 없이 `@seed-design/lynx-react` 패키지에서 직접 제공되는 package-only 공개 컴포넌트입니다. `ChipTabsList`를 `placement={["left", "right"]}`인 `ScrollFog`로 감싸면 기본 20px fog가 좌우 edge에 표시됩니다. List의 내부 좌우 padding 16px은 그대로 유지합니다.
