...
```tsx
import { BottomSheet } from "@seed-design/lynx-react";
      ...
      <BottomSheet.Positioner>
        ...
        <BottomSheet.Content>
          ...
          <BottomSheet.Header>
            ...
            <BottomSheet.Description>부가 설명</BottomSheet.Description>
          </BottomSheet.Header>
          <BottomSheet.Body>
            <text>본문 콘텐츠</text>
          </BottomSheet.Body>
...
`BottomSheet.Positioner`는 시트가 처음 열릴 때까지 `BottomSheet.Backdrop`과 `BottomSheet.Content`의 마운트를 지연시킵니다. `BottomSheet.Trigger`는 Positioner 밖에 두어야 탭으로 시트를 열 수 있습니다.

### Imperative 제어

`BottomSheet.Root`에 `ref`를 전달하면 `open`, `close`, `snapTo`, `expand`, `collapse` 메서드를 호출할 수 있습니다.

```tsx
import { useRef } from "@lynx-js/react";
import { BottomSheet, type BottomSheetRootRef } from "@seed-design/lynx-react";

export function App() {
  const ref = useRef<BottomSheetRootRef>(null);

  return (
    <BottomSheet.Root ref={ref} snapPoints={["fit", "80%"]}>
      {/* ... */}
    </BottomSheet.Root>
  );
}
