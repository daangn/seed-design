...
가장자리에 놓인 아이콘 버튼(왼쪽 슬롯의 첫 번째, 오른쪽 슬롯의 마지막)은 아이콘 버튼의 투명 여백(터치 타겟이 아이콘보다 큼)만큼 자동으로 바깥으로 당겨져 아이콘이 화면 콘텐츠 여백에 정렬됩니다. `AppBarLeft` / `AppBarRight`가 가장자리 위치를 Context로 전달하므로 별도 prop 없이 동작하며, `AppBarBackButton` / `AppBarCloseButton`뿐 아니라 직접 만든 `AppBarIconButton`에도 동일하게 적용됩니다. 보정은 아이콘 버튼에만 적용되어 `AppBarSlot`(custom)이나 인접한 다른 버튼은 영향을 받지 않습니다.
