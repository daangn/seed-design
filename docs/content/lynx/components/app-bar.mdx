...
- **가장자리 아이콘 정렬**: 웹은 `iconButton:first-child` / `:last-child` selector로 bleed를 보정하지만, Lynx는 해당 가상 클래스를 지원하지 않아 `AppBarLeft` / `AppBarRight`가 가장자리 아이콘 버튼에 `edge` prop을 자동 주입해 보정합니다. 다중 버튼·custom slot에서도 가장자리 아이콘 버튼에만 정확히 적용됩니다.
</Callout>
