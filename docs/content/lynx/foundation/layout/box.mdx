...
`Box`는 편의를 위한 primitive입니다. 많은 항목을 반복 렌더링하는 화면에서는 primitive 컴포넌트와 object style 계산 비용이 누적될 수 있으므로, 패키지 컴포넌트 내부나 성능에 민감한 리스트에서는 native `<view>`와 recipe className 또는 명시적인 style을 우선 사용합니다.
</Callout>
