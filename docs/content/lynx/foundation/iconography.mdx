...
<Callout>
  lynx 3.5 기준으로 SVG를 지원하지 않기 때문에 로더를 설정해 이를 WebP로
  변환하는 역할을 합니다. `@karrotmarket/lynx-*-icon` 패키지는 내부적으로
  `karrotmarket/assets-*` 패키지의 SVG를 사용하는데, 이를 로컬에서 `sharp`를
  사용해서 WebP로 변환하는 역할을 합니다.
</Callout>
