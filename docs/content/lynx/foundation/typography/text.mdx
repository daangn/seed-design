...
<Callout>
Lynx에서 `Text` 컴포넌트는 `text` 요소로 렌더링됩니다.
</Callout>
...
Lynx에서 SEED Design은 **Dynamic(sp)**과 **Static(px)** 두 종류의 타이포그래피 토큰을 제공합니다.

| 종류 | 단위 | 폰트 스케일링 | 사용 시나리오 |
|------|------|---------------|--------------|
| Dynamic (`t1Regular`, `t5Bold` 등) | `sp` | 시스템 폰트 크기에 반응 | 본문, 제목 등 일반 텍스트 |
| Static (`t1StaticRegular`, `t5StaticBold` 등) | `px` | 고정 크기 | 탭 바, 배지 등 레이아웃 고정 텍스트 |

<Callout>
`fontScaleEffectiveOnlyOnSp` feature flag가 활성화되어 있어야 합니다. 이 플래그가 켜지면 `sp` 단위만 시스템 폰트 크기 변경에 반응하고, `px` 단위는 고정됩니다. [Feature Flags 문서](/lynx/getting-started/feature-flags)를 참고하세요.
</Callout>
...
| 항목 | 웹 | Lynx |
|------|-----|------|
| Dynamic font-size | `clamp()` + `rem` | `sp` |
| Static font-size | `px` | `px` |
| Dynamic line-height | `clamp()` + `rem` | `sp` |
| Static line-height | `px` | `px` |
...
Dynamic textStyle은 `sp` 단위를 사용하므로 시스템 폰트 크기 변경에 반응합니다.
...
Static textStyle은 `px` 단위를 사용하므로 시스템 폰트 크기 변경에 반응하지 않습니다. 탭 바, 배지 등 레이아웃이 고정되어야 하는 텍스트에 적합합니다.

```tsx
import { Text, VStack } from "@seed-design/lynx-react";

export function StaticTextStylesExample() {
  return (
    <VStack gap="x1">
...
export function FontSizesExample() {
  return (
    <VStack gap="x1">
      <Text color="fg.neutral" fontSize="t1">fontSize t1 (dynamic, sp)</Text>
      <Text color="fg.neutral" fontSize="t3">fontSize t3 (dynamic, sp)</Text>
      <Text color="fg.neutral" fontSize="t5">fontSize t5 (dynamic, sp)</Text>
      <Text color="fg.neutral" fontSize="t7">fontSize t7 (dynamic, sp)</Text>
    </VStack>
  );
}
```
...
- **`as` 속성 미지원**: Lynx에서는 항상 `text` 요소로 렌더링되므로 `as`로 요소를 변경할 수 없습니다.
- **`maxLines` 미지원**: Lynx에서는 텍스트 줄 수 제한을 직접 지원하지 않습니다.
