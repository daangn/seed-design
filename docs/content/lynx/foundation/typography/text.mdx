...
<Callout>Lynx에서 `Text` 컴포넌트는 `text` 요소로 렌더링됩니다.</Callout>

## Props

<react-type-table
  path="../packages/lynx-react/src/components/Text/Text.tsx"
  name="TextProps"
/>
...
<Callout>
  `fontScaleEffectiveOnlyOnSp` feature flag가 활성화되어 있어야 합니다. 이
  플래그가 켜지면 `sp` 단위만 시스템 폰트 크기 변경에 반응하고, `px` 단위는
  고정됩니다. [Feature Flags 문서](/lynx/getting-started/feature-flags)를
  참고하세요.
</Callout>
...
export function ColorsExample() {
  return (
    <VStack gap="x1">
      ...
      <Text color="fg.success" textStyle="t5Regular">
        fg.success
      </Text>
    </VStack>
  );
