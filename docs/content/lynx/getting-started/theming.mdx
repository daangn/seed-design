...
런타임 값이 필요한 safe area는 테마 설정과 분리해서 다룹니다. safe area가 필요한 화면이나 컴포넌트에서 값을 직접 읽어 적용하세요.

## 전역 설정 적용

`@seed-design/lynx-react`의 `getSeedClassName()` 함수를 사용하여 `<page>` 요소에 테마/플랫폼 클래스를 설정합니다.
...
`getSeedClassName()`은 다음 값을 포함한 className 문자열을 반환합니다:
...
Safe area는 `getSeedClassName()`의 책임이 아닙니다. safe area 값이 필요한 화면이나 컴포넌트에서 `useSafeArea()`를 직접 사용하세요.
...
`useSafeArea()`는 `lynx.__globalProps.safeAreaInsetTop` / `safeAreaInsetBottom` 값을 우선 사용하고, host가 값을 제공하지 않으면 `env(safe-area-inset-top)` / `env(safe-area-inset-bottom)` 값을 fallback으로 반환합니다.

`Box`에서는 `pt="safeArea"` / `pb="safeArea"`를 사용할 수 있습니다. `BottomSheet.Content`처럼 safe area가 컴포넌트 구조와 직접 연결된 경우에는 컴포넌트 내부에서 필요한 bottom safe area를 처리합니다.
...
`getSeedClassName()`은 `SystemInfo.platform`을 읽어 플랫폼 클래스도 함께 반환합니다.
