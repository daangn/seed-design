...
/** 블로그형 카드: 16:9 썸네일 + 날짜/제목/설명. internal=→, external=↗ 화살표. */
    ...
    <>
      ...
      <div className="mt-1 flex items-start justify-between gap-2">
        ...
        <IconSeedArrow
          ...
          className={clsx(
            "mt-1 size-4 text-fd-muted-foreground",
            card.external && "-translate-y-[0.5px]",
          )}
        />
      </div>
      {card.description && (
...
/**
 * Updates 섹션 랜딩 — 업데이트/릴리스 글을 블로그형 카드 그리드로. 사이드바 없는 1컬럼.
 * 내부 글(→)과 외부 팀블로그 글(↗)을 링크 종류에 맞는 화살표로 구분한다.
 */
  ...
  return (
    <ProsePage title={UPDATES_TITLE} description={UPDATES_DESCRIPTION}>
      ...
      <section className="not-prose">
        <h2 className="mb-5 text-xl font-semibold tracking-tight text-fd-foreground md:mb-6 md:text-2xl">
          Blog
        </h2>
