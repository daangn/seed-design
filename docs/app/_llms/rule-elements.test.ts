...
describe("RULE_ELEMENT_NAMES", () => {
  ...
  // 부모가 탭 이름을 잃는다. 핸들러를 두지 않기로 한 이름은 ELEMENTS_WITHOUT_RULE이 따로 센다.
  const PRESERVED_WITHOUT_OWN_HANDLER = ["CodeBlockTab", ...ELEMENTS_WITHOUT_RULE];
  ...
  it("아무도 주장하지 않는 태그를 남겨 두지 않는다", () => {
    ...
    expect(unclaimed.sort()).toEqual([...PRESERVED_WITHOUT_OWN_HANDLER].sort());
  });
});
