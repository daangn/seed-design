...
`docs/app/_llms`는 llms.txt로 내보낼 **섹션의 정의와 라우트**를 담는다. MDX를 LLM 친화 텍스트로 바꾸는 일 자체는 컴파일 타임에 `lib/llms`의 핸들러가 하고(`lib/llms/AGENTS.md` 참조), 이 폴더는 그 결과를 어떤 주소로 어떻게 묶어 내보낼지를 정한다. 입력인 `processed` 익스포트는 상위 `app/source.tsx`가 만든다.

## 파일 작성 컨벤션

- `config.ts`가 섹션 레지스트리의 단일 진입점이다. 섹션의 콘텐츠 디렉토리·URL·라벨·설명·그룹핑·스니펫 레지스트리를 한곳에 적는다.
- 라우트 팩토리는 역할 이름으로 둔다(`llms-route.ts`는 페이지별 `.txt`, `section-index.ts`는 섹션 목록).
...
- 라벨·설명·URL을 라우트에 손으로 적지 않는다. 전부 레지스트리에서 읽는다 — 손으로 적은 링크가 IA 개편 때 통째로 썩었다.
- 섹션 소스는 비동기 getter다(`getReactSource()`). 팩토리에는 호출 결과가 아니라 getter를 넘긴다.
...
- 섹션 인덱스 라우트가 내보내는 링크는 페이지 URL이 아니라 `/llms/{...}.txt`다. 이 사이트는 정적 익스포트라 `Accept` 협상이 없어, 페이지 URL은 HTML로 응답한다.
