import { type Section, getLLMMarkdownUrl, shouldIncludeInFullText } from "./config";
 ...
 * 카탈로그 셸이라 제외된 섹션에서는 빈 문자열이 되어 아무것도 추가하지 않는다.
 */
export function sectionOverviewLine(
  ...
  const page = pages.find((p) => p.slugs.length === 0 && shouldIncludeInFullText(section, p.path));
  if (!page) return "";
