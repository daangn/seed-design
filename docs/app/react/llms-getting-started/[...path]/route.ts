...
/**
 * Generate static params for getting started documentation pages.
 */
export async function generateStaticParams(): Promise<StaticParams[]> {
  const gettingStartedPageSlugs = reactSource
    .getPages()
    .filter((page) => {
  ...
  return gettingStartedPageSlugs;
}
...
export async function GET(_: Request, { params }: { params: Promise<StaticParams> }) {
  ...
  // Try to find in getting-started
  const page = reactSource.getPage(["getting-started", ...slugsExtensionRemoved]);
