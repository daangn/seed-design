...
/**
 * Generate static params for updates documentation pages.
 */
export async function generateStaticParams(): Promise<StaticParams[]> {
  const updatesPageSlugs = reactSource
    .getPages()
    .filter((page) => {
      const [firstSlug] = page.slugs;
      // Exclude the main changelog page as it has its own dedicated endpoint
      if (page.path.includes("changelog.mdx")) return false;
...
export async function GET(_: Request, { params }: { params: Promise<StaticParams> }) {
  ...
  // Try to find in updates
  const page = reactSource.getPage(["updates", ...slugsExtensionRemoved]);
