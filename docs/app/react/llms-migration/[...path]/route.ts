...
/**
 * Generate static params for migration documentation pages.
 */
export async function generateStaticParams(): Promise<StaticParams[]> {
  const migrationPageSlugs = reactSource
    .getPages()
    .filter((page) => {
...
export async function GET(_: Request, { params }: { params: Promise<StaticParams> }) {
  ...
  // Try to find in migration
  const page = reactSource.getPage(["migration", ...slugsExtensionRemoved]);
