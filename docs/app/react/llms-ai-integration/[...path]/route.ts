...
/**
 * Generate static params for AI integration documentation pages.
 */
export async function generateStaticParams(): Promise<StaticParams[]> {
  const aiIntegrationPageSlugs = reactSource
    .getPages()
    .filter((page) => {
...
export async function GET(_: Request, { params }: { params: Promise<StaticParams> }) {
  ...
  // Try to find in ai-integration
  const page = reactSource.getPage(["ai-integration", ...slugsExtensionRemoved]);
