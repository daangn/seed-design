  ...
  return (
    <div className="flex flex-col gap-6">
      {groupedEntries.map((group) => {
        ...
        const groupQueryHref = `/react/updates/changelog?package=${encodeURIComponent(group.packageName)}&exact=${encodeURIComponent(group.version)}`;
          ...
          >
            <div className="sticky top-16 z-10 flex items-center justify-between gap-2 flex-wrap border-b border-fd-border px-4 py-2.5 bg-fd-card/95 backdrop-blur supports-[backdrop-filter]:bg-fd-card/80">
              ...
              <div className="inline-flex items-center gap-2">
                <a
                  href={groupAnchorHref}
                  className="text-[11px] font-medium text-fd-muted-foreground hover:text-fd-foreground underline-offset-2 hover:underline"
                  aria-label={`${group.packageName}@${group.version} 딥링크`}
                >
                  링크
                </a>
                            ...
                            {uniqueAdditionalPackages.map((pkg) => {
                              ...
                              return (
                                <a
                                  ...
                                  href={`/react/updates/changelog?package=${encodeURIComponent(pkg.name)}&exact=${encodeURIComponent(pkg.version)}#${targetAnchorId}`}
