...
/** SEED identity dot → expanded ring. Blog cards morph it into a titled chip. */
 ...
 * clickable elements and, over blog cards, becomes a chip showing the post title on
 * a random brand color. Desktop + fine-pointer + motion only; otherwise the native
 * cursor is kept and nothing mounts behaviorally.
 *
 * Position is driven with GSAP `quickTo` (no re-render on move); only the mode /
 * text / color use state, and only change on hover enter/leave.
 */
export function CustomCursor() {
  ...
  // Size morph on mode change (low frequency).
  useGSAP(() => {
    const el = dotRef.current;
    if (!el) return;
    const size = mode === "dot" ? 12 : mode === "expand" ? 56 : 180;
  ...
  return (
    <div
      ref={dotRef}
      aria-hidden="true"
      className="pointer-events-none fixed top-0 left-0 z-[9999] hidden items-center justify-center rounded-full will-change-transform [@media(pointer:fine)]:flex"
      style={{
        backgroundColor: mode === "text" ? bg : "#ffffff",
        // difference keeps the dot/ring visible on any background; the titled chip
        // shows its real brand color.
        mixBlendMode: mode === "text" ? "normal" : "difference",
      }}
    >
      {mode === "text" && (
        <span className="px-4 text-center font-bold text-sm text-white leading-tight">{text}</span>
      )}
    </div>
  );
}
