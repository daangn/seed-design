...
function parseErrorMessage(error: Error): string {
  ...
  return "응답 중 오류가 발생했어요. 다시 시도해보세요.";
}
...
export const ChatInterface = forwardRef<HTMLDivElement, Record<string, never>>(
    ...
    return (
      <div ref={ref} className="flex flex-col h-full bg-fd-background">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {error && (
            <div className="flex justify-start">
              <p className="text-sm text-fd-destructive">{parseErrorMessage(error)}</p>
            </div>
          )}
        </div>
