...
export function parseChangesetContent(file: string, content: string): ChangesetEntry {
...
export async function parseChangesetFile(file: string): Promise<ChangesetEntry> {
  return parseChangesetContent(file, await readFile(file, "utf8"));
}
...
export async function validateChangesets(
  ...
  lane: LaneName,
  expected: BumpType,
  readContent: (file: string) => Promise<string> = (file) => readFile(file, "utf8"),
  ...
  const entries = await Promise.all(
    changesetFiles.map(async (file) => parseChangesetContent(file, await readContent(file))),
  );
