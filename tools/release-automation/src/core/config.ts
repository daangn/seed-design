...
import type { BumpType, LaneConfig, LaneDefinition, LaneName, ReleaseControl } from "./types";
...
export function parseReleaseControl(value: unknown): ReleaseControl {
  ...
  const requiredKeys = ["$schema", "schemaVersion", "mode", "rootageContractReady"] as const;
  const allowedKeys = [...requiredKeys, "freeze"];
  const actualKeys = Object.keys(value);
  const missing = requiredKeys.filter((key) => !actualKeys.includes(key));
  const unknown = actualKeys.filter((key) => !allowedKeys.includes(key));
  if (missing.length > 0 || unknown.length > 0) {
    throw new Error(
      `릴리즈 제어 설정 키가 스키마와 다릅니다. 누락: ${missing.join(", ") || "없음"}, 알 수 없음: ${unknown.join(", ") || "없음"}`,
    );
  }
  if ("freeze" in value && value.freeze !== null) {
    throw new Error("레거시 freeze 필드는 null만 허용합니다.");
  }
