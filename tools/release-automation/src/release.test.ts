...
import { parseChangesetFile, validateChangesets } from "./core/changesets";
...
import type { LaneConfig, PullRequestIdentity, ReleaseControl, ReleaseMarker } from "./core/types";
import { assertStableVersionsAdvance, authorizePublish, compareSemver } from "./publish/publish";
import {
  idempotencyKey,
  isProcessed,
...
describe("릴리즈 설정", () => {
  test("세 고정 레인을 읽는다", () => {
    ...
    expect(parseReleaseControl({ ...control, freeze: null })).toEqual(control);
    expect(() => parseReleaseControl({ ...control, freeze: { lane: "minor" } })).toThrow(
      "freeze 필드는 null",
    );
  });
...
describe("릴리즈 PR CLI", () => {
  test("비릴리즈 레인 PR은 성공으로 건너뛴다", async () => {
    ...
    const process = Bun.spawn(
      ["bun", "tools/release-automation/bin/control.ts", "validate-pr", "--event", eventPath],
...
describe("자동화 PR marker", () => {
  ...
  test("실제 generator branch schema만 marker 필드와 정확히 결속한다", () => {
    const activation: ReleaseMarker = {
      schemaVersion: 1,
      type: "activation",
      lane: "dev",
      tag: "enable-sync",
    };
    expect(hasExpectedGeneratedHeadRef("release-activation/enable-sync-123", activation)).toBe(
      true,
    );
    expect(
      hasExpectedGeneratedHeadRef("release-activation/enable-dry-run-123", {
        ...activation,
        tag: "enable-dry-run",
      }),
    ).toBe(true);
    expect(
      hasExpectedGeneratedHeadRef("release-activation/enable-sync-123-spoof", activation),
    ).toBe(false);
    expect(
      hasExpectedGeneratedHeadRef("release-activation/start-123", {
        ...activation,
        targetLane: "major",
        tag: "start",
      }),
    ...
    expect(hasExpectedGeneratedHeadRef("release-sync/minor-42-to-major-attempt-2", sync)).toBe(
      true,
    );
    expect(hasExpectedGeneratedHeadRef("release-sync/minor-42-to-major-attempt-1", sync)).toBe(
      false,
    );

    const bootstrap: ReleaseMarker = {
      schemaVersion: 1,
      type: "bootstrap",
      lane: "minor",
      targetLane: "minor",
    ...
    expect(hasExpectedGeneratedHeadRef("release-bootstrap/minor-123", bootstrap)).toBe(true);
    expect(hasExpectedGeneratedHeadRef("release-bootstrap/minor-123-spoof", bootstrap)).toBe(false);
  });

  test("지원하지 않는 generated PR marker 유형을 거부한다", () => {
    expect(
      parseMarker(
        `<!-- seed-release:${JSON.stringify({ schemaVersion: 1, type: "unsupported", lane: "major" })} -->`,
      ),
    ).toBeNull();
  });
});
...
describe("동기화 queue", () => {
  ...
  test("conflict reject 파일은 target worktree에서만 삭제한다", async () => {
    const dev = await mkdtemp(join(tmpdir(), "seed-sync-dev-"));
    const target = await mkdtemp(join(tmpdir(), "seed-sync-target-"));
    temporaryDirectories.push(dev, target);
    await Promise.all([
      writeFile(join(dev, "conflict.rej"), "dev"),
      writeFile(join(target, "conflict.rej"), "target"),
    ]);

    await removeSyncRejectFiles(target, ["conflict.rej"]);

    expect(await readFile(join(dev, "conflict.rej"), "utf8")).toBe("dev");
    expect(await Bun.file(join(target, "conflict.rej")).exists()).toBe(false);
    await expect(removeSyncRejectFiles(target, ["../outside.rej"])).rejects.toThrow("worktree 밖");
  });
});
