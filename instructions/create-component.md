...
- `Optional`은 `Note`를 참고해 필요하지 않은 경우 생략해도 됩니다.

## (Optional) 1. Headless 작성
- **Path**: `@packages/react-headless/[$component-name]`
...
- **Description**: registry entry 파일
- **Note**: 새로운 컴포넌트 추가 시 필요

## (Optional) 6-2. registry generate
- **Path**: `@docs`
- **Command**: `bun generate:registry`
- **Description**: 문서 사이트에서 사용할 registry 파일 생성
- **Note**: 새로운 컴포넌트 추가 시 필요

## 7. 컴포넌트 예시 생성
- **Path**: `@docs/components/example/[$component-name]-*.tsx`
- **Examples**: preview, size variations, variants, usage patterns
- **Description**: 컴포넌트에 대한 Variant 사용법, 특수 케이스, Preview 컴포넌트 등
- **Note**: 새 컴포넌트 추가 시 필요

## 8. Storybook 컴포넌트 추가
- **Path**: `@docs/stories/[$component-name].stories.tsx`
