...
target platform이 `lynx` 또는 `cross-platform`의 Lynx 구현이면 이 문서를 token vocabulary 비교용으로만 사용하고, 실제 recipe 구현은 [lynx-patterns.md](lynx-patterns.md)와 `packages/lynx-qvism-preset/AGENTS.md`를 따른다.
