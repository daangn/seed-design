...
[컴포넌트 경로 조회](component-map.md)가 찾는 실제 경로만 읽어 한 컴포넌트의 React와 Lynx 공개 표면을 비교한다. 파일을 만들거나 수정하지 않는다.

## 실행

정확한 컴포넌트 이름이나 현재 표면이 불확실하면 먼저 경로를 조회한다. 저장소 안에서 컴포넌트 하나를 지정한다.
...
3. `platformDifferences.expected`에서 현재 Lynx 구현이나 문서에 명시된 플랫폼 제약을 확인한다. 현재 컴포넌트의 Lynx 소스나 문서에서 제약을 명시적으로 확인했을 때만 `expected`로 본다.
4. `platformDifferences.needsReview`에서 한쪽에만 관찰된 값을 직접 검토한다. `possiblyExplainedBy`는 관련 있을 수 있는 `expected` ID와 그 규칙에 이름이 일치한 양쪽 값을 함께 반환한다. 판정이 아니라 검토 단서이므로 근거 경로와 대체 동작을 직접 읽는다.

`dimensions`는 원시 비교 결과를 유지한다. `needsReview`도 한쪽 관찰값을 빠뜨리지 않고 유지하며, `possiblyExplainedBy`가 있어도 검토 목록에서 제거하지 않는다. 브라우저 DOM과 Lynx 네이티브 환경의 차이로 설명할 수 있는지, 대체 동작이 같은 사용자 결과를 내는지 확인한 뒤 미구현 여부를 판정한다.
...
`unknown`과 `needsReview`는 누락의 증거가 아니다. 상속 타입과 대체 동작을 확인한 뒤 판정한다.
...
플랫폼 제약 후보가 있으면 세부 판정 기준은 [React와 Lynx 플랫폼 차이 판정 기준](platform-differences.md)을 따른다.

## 결과 사용

비교만 요청받았다면 공통·의도한 플랫폼 차이·보완할 누락·미확인 항목과 근거 경로를 반환하고 끝낸다. 구현 요청 안에서 사용했다면 확인한 차이를 [플랫폼 선택](platform-gate.md)과 [API 설계](api-design.md)의 배포 표면 결정에 입력한다.
