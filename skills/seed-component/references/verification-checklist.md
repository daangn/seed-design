...
- [Lynx 문서·예제 작업](lynx-docs.md#배포-경로-확인)에서 확정한 배포 경로가 Registry, 문서, SPA 예제에서 일치하는지 확인한다.
...
- [ ] 공개 패키지 변경이면 [`seed-change`](../../seed-change/SKILL.md)의 changeset 분기로 버전 후보와 한국어 changeset을 확인했는가?
- [ ] `seed-change`의 계획 분기로 영향 범위, 검증 순서, `origin/dev`·`origin/minor`·`origin/major` 중 PR base를 정했는가?
- [ ] 제출을 요청받았다면 `seed-change`의 제출 분기가 같은 base로 rebase·commit·push·PR을 준비하는가?
