...
Lynx 구현은 React Web 패턴의 변형이 아니라 별도 런타임을 대상으로 한다. CSS-free Headless와 Styled Primitive의 경계를 지키고, Web에서 가능한 DOM, CSS, SVG, form/focus 모델이 Lynx에서 그대로 동작한다고 가정하지 않는다.

## 책임 분리

Stateful Lynx 컴포넌트는 아래 경계를 지킨다. 이 표가 Headless·Styled Primitive·Registry의 단일 책임 기준이다.

| 레이어 | 책임 | 넣지 않는 것 |
|--------|------|--------------|
| `packages/lynx-react-headless/*` | CSS-free 상태 전이, press/tap, controlled/uncontrolled, context, 이벤트 순서, ref, 접근성 계약, 필수 native 구조, 기능에 필요한 geometry(측정·clipping·hidden·초기/업데이트 수명) | SEED recipe/token/시각 variant 계산, 시각 motion, 권장 문구·icon·조립 |
| 기존 외부 Lynx primitive | primitive가 이미 소유한 상태·이벤트 | Headless나 wrapper의 중복 상태 |
| `packages/lynx-react/*` Styled Primitive | Headless 계약 소비, native presentation slot, recipe variant/className, divider·chevron·시각 motion | 상태·이벤트·ref·a11y 재구현, 기능 geometry 재계산, Web DOM/form/focus API |
...
| `docs/registry/lynx/ui/*` | 권장 문구, icon 선택·연결, 편의 조립 | package 동작·측정·숨김·ref의 재구현 |

`packages/lynx-react-headless/image`, `toggle`, `use-controllable-state`, `use-press-tap`가 기존 기준이다. 이번 Accordion은 `@seed-design/lynx-react-accordion`으로 같은 경계를 적용한다. Headless 공개 API는 필요한 경우 namespace와 named component를 함께 제공하며, 모든 컴포넌트를 일률적인 hook 하나로 공개하지 않는다.

Headless package는 source, runtime, declaration graph를 CSS 없이 독립 소비할 수 있어야 한다. Headless package 자체의 type/build와 실제 consumer의 type/build는 native 장면 검증과 별도 결과로 기록한다. native 검증은 `examples/lynx-spa`에서 실제 bundle을 실행한 경우에만 완료로 기록한다.

DES-2608 범위에서 금지하는 것은 Headless의 SEED recipe/token 의존과 시각 variant/class 계산이다. 사용자가 전달하는 generic `className`/`style`과 이벤트/ref 합성은 공개 props contract가 허용하는 범위에서 사용할 수 있다.

기능 geometry와 표현 geometry를 나눈다. Accordion Content의 높이 측정·clipping·hidden은 기능 geometry이지만, FAB나 ProgressCircle의 크기·정렬처럼 표현을 위한 geometry는 Styled Primitive에 남긴다. 모든 geometry를 Headless로 옮기지 않는다.

공개 API는 실제 재사용 계약에 맞춘 component와 props로 정한다. 일률적인 hook 공개나 빈 계층을 만들지 않고, `Host`·`asChild`·`renderRoot` escape hatch는 Lynx runtime 계약이 있을 때만 둔다. Headless에는 기능 geometry만 두고 모든 geometry를 옮기지 않으며, Registry wrapper도 권장 조립을 짧게 만들 때만 둔다.
...
- **Headless package**: 상태에 따라 필요한 `accessibility-*` 속성과 native 상호작용 구조를 소유하고, `expanded`/`collapsed` 같은 상태값과 소비자 label override를 명시적 계약으로 제공한다. 상태를 Styled Primitive에서 다시 계산하지 않는다.
- **기존 외부 primitive**: a11y를 포함한 상태 계약을 이미 소유하면 그 계약을 사용한다. wrapper가 같은 속성을 중복 계산하지 않는다.
- **Styled Primitive**: Headless가 만든 a11y 계약과 소비자 override를 native slot에 전달하고 recipe를 조합한다. 역할·상태·이벤트·ref는 Headless 계약을 그대로 소비한다.
- 공통 a11y helper를 새로 만들지 않는다. 컴포넌트별 native slot과 공개 계약에 필요한 속성만 Headless에 직접 둔다.
...
```tsx
// Headless interactive slot — 필수 a11y를 native <view>에 직접 작성
<view
  {...pressHandlers}
...
`accessibility-value`/`accessibility-label`의 상태 문구는 Headless의 기본 계약과 소비자 override로 정한다. "켜짐"/"꺼짐", "expanded"/"collapsed"처럼 locale이 필요한 값은 소비자가 제공하거나 Headless의 명시적 accessibility prop으로 전달한다.
...
- Delivery Surface 결정은 `api-design.md`, Lynx 문서와 실행 예제의 작성 규칙은 [Lynx 문서·예제 작업](lynx-docs.md)을 따른다.
