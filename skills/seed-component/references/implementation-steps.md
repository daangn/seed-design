...
[경로 조회](component-map.md)와 [API 비교](api-parity.md)는 탐색의 시작점이다. 맵에 없는 hook·context·유틸리티나 상속된 API가 있을 수 있으므로, 공개 컴포넌트의 import와 호출을 따라 실제 행동을 만드는 원천까지 읽는다. 예제 JSX나 같은 prop 이름만으로 동작을 추정하지 않는다. 외부 라이브러리가 행동을 소유하면 사용 중인 옵션과 버전의 문서·소스에서 필요한 계약을 확인한다.
...
2. 그 장면에 필요한 원천·생성물·공개 export·Registry·기존 예제 또는 호스트 경로만 연결한다. Lynx native 장면은 `examples/lynx-spa`의 문서 예제 경로를 사용한다. package나 Registry를 우회하는 검증용 구현을 만들지 않는다.
3. 안정된 변경본에서 필요한 검사별 실행자를 정하고 [관찰 가능한 결과 판정](verification-checklist.md#관찰-가능한-결과-판정)으로 확인한다. 입력·산출물·실행 자원이 독립적인 검사는 병렬로 실행한다. 실패하면 해당 원천을 고친 뒤 영향받는 검사와 장면만 다시 확인한다.
...
Lynx 기본 장면은 `examples/lynx-spa`의 문서 예제와 query를 포함한 실제 bundle URL로 native 결과를 먼저 확인한다. 구체적인 실행·주소·딥 링크·환경 판정은 [`검증 런북`](../../seed-verify-lynx-component/references/verification.md)에만 따른다. MDX 페이지·`LynxComponentExample` host·코드 탭·QR·Web preview·docs build pipeline 자체가 검증 대상인 경우에만 필요한 문서 출력을 준비한다. 그 외에는 전체 docs 빌드나 정적 bundle 서빙을 기본 장면의 선행 조건으로 두지 않는다. 관련 문서 검증은 해당 변경이 모인 안정된 차수에서 수행하며, 작은 원천 수정마다 무조건 반복하지 않는다.
...
- Lynx: `packages/lynx-react-headless/[name]/` (CSS-free Headless); 기존 외부 Lynx primitive가 상태를 이미 소유하는 경우에만 `packages/lynx-react/src/hooks/` 또는 컴포넌트 내부 hook/context
...
Stateful Lynx 컴포넌트는 [lynx-patterns.md](lynx-patterns.md)의 `책임 분리`를 따른다.

CSS-free Headless 구현·추출 또는 Styled 어댑터의 소비 계약을 바꿀 때는 [Lynx Headless 구현과 추출](lynx-headless.md)에서 관련 계약의 구현·검증 절차를 따른다.

- `packages/lynx-react-headless/[name]`은 [lynx-patterns.md](lynx-patterns.md)의 `책임 분리`에 맞는 CSS-free 공개 계약을 구현한다. API는 필요에 따라 namespace와 named component를 제공하며, 일률적인 hook 하나로 일반화하지 않는다.
- Styled UI는 Headless 계약을 소비하고, Registry는 권장 조립을 제공한다. 각 레이어의 세부 책임과 source/runtime/declaration graph·검증 경계는 [lynx-patterns.md](lynx-patterns.md)를 따른다.
- 이번 Accordion의 Content처럼 높이 측정·clipping·hidden·초기/업데이트 수명 등 기능 geometry는 Headless 계약에 포함한다. recipe/variant와 시각 motion은 Styled Primitive에 둔다.
...
- Lynx `## Usage`의 import 경로는 [Lynx 문서·예제 작업](lynx-docs.md#배포-경로-확인)의 배포 경로 규칙을 따른다.
...
- 상세 작성 규칙은 [Lynx 문서·예제 작업](lynx-docs.md)을 따른다.
...
React snippet 레이어가 있는 경우 `seed-design/ui/[name]`에서 import하고, Layout 컴포넌트(Flex, VStack 등)는 `@seed-design/react`에서 import한다. Lynx 문서와 앱 예제는 [Lynx 문서·예제 작업](lynx-docs.md#배포-경로-확인)에서 확정한 배포 경로를 따른다.
...
근거가 없는 `unknown`은 해당 시나리오를 구현하기 전에 해결한다. 미지원 기능은 실행 예제를 만들지 않고 구현체 부재 근거와 앱 수준 대안을 문서에 남긴다. 상세 형식은 [Lynx 문서·예제 작업](lynx-docs.md#react-문서와-맞추기)을 따른다.
