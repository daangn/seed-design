...
Lynx 문서와 예제는 같은 컴포넌트의 React 문서를 기준으로 작성한다. 지원되는 기능은 섹션 순서, 예제 제목, 시나리오, 사용자가 보는 결과를 가능한 한 같게 유지한다. Lynx 런타임 때문에 달라지는 부분만 바꾸고 이유를 적는다. frontmatter·MDX·예제 엔트리의 세부 형식은 [Lynx 문서 작성 규칙](lynx-docs-authoring.md)에 있다.
...
3. package export·Registry·문서·예제 경로가 불명확하거나 새 시나리오를 추가한다면 [컴포넌트 경로 조회](component-map.md)로 필요한 경로만 찾고 결과 파일을 읽는다.
4. React와 Lynx 공개 API 차이가 문서 구성이나 사용자 결과에 영향을 줄 때만 [React·Lynx API 비교](api-parity.md)를 사용한다. 플랫폼 제약으로 의도한 차이와 보완할 누락을 구분한다.

## 검증 범위 정하기

- 새 컴포넌트·생성물·공개 소비 경로를 추가할 때만 [`seed-change`](../../seed-change/SKILL.md)의 계획으로 target branch와 release lane을 확인한다. 알려진 문서의 국소 수정에는 적용하지 않는다.
...
포팅 중인 컴포넌트를 소비한다면 [참조 동작 추적과 기본 장면](implementation-steps.md#참조-동작-추적과-기본-장면)의 검증 결과를 확인한 뒤 그 동작에 의존하는 변형 예제를 확장한다. Lynx 기본 장면은 필요한 package·생성물과 실제 공개 소비 경로를 `examples/lynx-spa`의 문서 예제로 연결해 먼저 확인한다. MDX host·QR·Web preview 같은 문서 자체를 바꾸지 않았다면 전체 docs 빌드나 정적 bundle 서빙을 선행 조건으로 두지 않는다. 시나리오 대응표 작성은 병렬로 진행할 수 있다. 기본 장면 실패를 예제 wrapper나 frame 조정으로 숨기지 말고 원천 담당에게 돌려보낸다.
...
3. 컴포넌트 사용법, Engine·XElement, CSS·element 사용을 새로 추가하거나 바꿀 때만 [작성 규칙](lynx-docs-authoring.md)의 호환성 절차와 `find-refer`·`lynx-api-docs`·`lynx-check-css-support`를 사용한다.
4. [작성 규칙](lynx-docs-authoring.md)에 따라 필요한 frontmatter, MDX, 예제 엔트리를 작성한다.
...
- 실제 호스트 앱에서도 재현되는 API나 동작 문제는 [기존 구현 변경](implementation-workflow.md#기존-구현-변경)으로 분리한다. 문서용 우회를 만들지 않는다.
