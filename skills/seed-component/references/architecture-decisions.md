...
| Current surface | [경로 조회](component-map.md)의 matched / ambiguous / not-found와 근거 경로 |
| API parity | cross-platform이면 [API 비교](api-parity.md) 차이와 직접 확인이 필요한 unknown 항목 |
...
**Lynx Headless와 상태 책임**:
`packages/lynx-react-headless/*`의 기존 기준은 `image`, `toggle`, `use-controllable-state`, `use-press-tap`이며, 이번 승인된 Accordion은 `@seed-design/lynx-react-accordion`으로 같은 경로를 따른다. 공개 API는 필요한 경우 namespace와 named component를 함께 제공하며, 모든 컴포넌트를 hook 하나로 만들지 않는다.

레이어별 책임과 CSS-free graph·검증 경계는 [lynx-patterns.md](lynx-patterns.md)의 `책임 분리`를 따른다. 신규 패키지는 기존 경로로 재사용 계약을 충족할 수 없을 때만 사용자에게 먼저 확인한다.

- **재사용 가능**: 기존 headless package, hook, context, primitive를 그대로 사용
- **확장 필요**: 기존 책임 경계를 유지하면서 상태 조합이나 slot contract를 추가
...
이번 변경에서 새로 도입하거나 아직 안정성이 확인되지 않은 의존성만 확인한다. 이미 사용 중인 안정된 내부 경로를 다시 전수 조사하지 않는다.

| 의존성 | API 안정? | 처리 |
|--------|----------|------|
| (필요한 경우에만 기록) | Yes/No | 사용 / 기존 안정 경로 선택 / 차단 사유 보고 |

불안정한 의존성이면 기존 안정 경로로 요구사항을 충족할 수 있는지 먼저 판단한다. 새 패키지·외부 의존성 추가처럼 경계를 넓혀야 할 때만 사용자 확인을 받고, 그렇지 않으면 근거 있는 구현 결정을 진행한다.
...
- **Headless reference**: ________________
  - Lynx 경로: `packages/lynx-react-headless/{name}/` 또는 기존 외부 primitive가 상태를 소유할 때만 `packages/lynx-react/src/hooks/`, 컴포넌트 내부 hook/context
...
새 컴포넌트나 Headless·Recipe·Registry 책임이 바뀌는 경우, 플랫폼과 배포 방식을 확정한 뒤 `scripts/scaffold-plan.ts`를 실행한다. 새 패키지·외부 의존성·CI 설정처럼 저장소 경계를 넓히는 결정에만 먼저 사용자 확인을 받는다. 결과에서 이번 작업에 필요한 항목만 확인한다.
