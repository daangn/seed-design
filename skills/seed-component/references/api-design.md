...
React와 Lynx를 함께 다루면 먼저 [React·Lynx API 비교](api-parity.md)의 스크립트를 실행한다. 결과의 `sources`와 차원별 `evidence`를 매트릭스 근거로 붙인다. `confidence: unknown`인 차원은 한쪽 기능이 없다는 뜻이 아니다. 해당 플랫폼의 공개 파일과 상속 타입을 직접 확인할 항목으로 남긴다.
...
- [API 비교](api-parity.md)에서 확인한 공통, React 전용, Lynx 전용 항목
