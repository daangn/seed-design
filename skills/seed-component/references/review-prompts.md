...
`seed-component`를 수정한 뒤 아래 프롬프트로 라우팅을 검토한다. 각 답변이 [경로 조회](component-map.md)로 현재 상태를 확인하고, 필요한 스킬과 reference만 선택하는지 본다.
...
확인할 것:
- 기본 장면은 필요한 package·생성물과 실제 공개 소비 경로로 확인한다. Lynx는 `examples/lynx-spa`의 문서 예제를 기본으로 사용하며, 전체 문서 빌드나 정적 bundle 서빙을 선행하지 않는다.
...
- 조율자가 모든 검사를 직접 실행하지 않고 결과를 통합·승인한다. 실패 원천과 수정 영향이 있는 검사만 다시 실행한다. Lynx 문서 페이지·QR·Web preview·빌드 경로 자체를 바꾼 경우에는 해당 문서 검증도 유지한다.
...
- BG-only와 소비자 MT 핸들러가 있는 경로를 나눠 pressed, end/cancel, disabled, 콜백 순서를 확인한다.
- ref는 실제 native 작업 결과로, 독립 소비는 package 공개 export와 타입 선언까지 확인한다.
- 같은 조건의 추출 전후 Recipe 계산량을 비교하되 프레임 시간이나 FPS 결과로 해석하지 않는다.
