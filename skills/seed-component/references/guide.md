...
| Headless·상태 | `packages/react-headless/` | `packages/lynx-react-headless/`; 기존 외부 Lynx primitive가 상태를 소유하는 경우에만 `packages/lynx-react/src/hooks/` 또는 Styled UI 내부 hook/context | 대상 패키지 테스트 |
...
- 상태·이벤트·ref·접근성·기능 geometry를 재사용해야 할 때만 Headless를 추가한다. Lynx의 Headless·Styled Primitive·Registry 책임은 [lynx-patterns.md](lynx-patterns.md)의 `책임 분리`를 따른다.
- 토큰이나 Recipe가 바뀌지 않으면 Rootage와 preset을 건드리지 않는다.
- Registry 필요 여부와 배포 방식은 [API 설계](api-design.md)에서 정한다. Lynx 문서와 예제의 소비 경로는 [Lynx 문서·예제 작업](lynx-docs.md)을 따른다.

구조가 애매하면 [경로 조회](component-map.md)로 현재 컴포넌트와 가까운 참조 컴포넌트를 먼저 비교한다. React와 Lynx를 함께 맞출 때는 [API 비교](api-parity.md)로 공개 차이를 확인한다.
...
변경 범위에 맞는 명령과 화면 확인은 [verification-checklist.md](verification-checklist.md)에서 고른다. 공개 패키지를 바꿨다면 [`seed-change`](../../seed-change/SKILL.md)의 changeset·계획 분기로 버전과 PR base를 정한다. 사용자가 제출을 요청한 경우에만 제출 분기로 이어간다.
