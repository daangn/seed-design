...
연결할 기기나 앱 세션이 없으면 실기기 확인을 환경 차단으로 보고한다. 이를 보완하려고 문서용 우회 구현이나 별도 실행 환경을 추가하지 않는다.
...
1. 작성하거나 수정한 문서와 예제
2. 웹 미리보기에서 확인한 항목
3. `.lynx.bundle` 파일과 매니페스트 항목 생성 여부
4. QR의 직접 bundle URL과 Explorer 버튼의 `lynx://open` 주소 확인 여부
5. Lynx Explorer 또는 PlayLynx의 실기기 확인 결과. 수행하지 못했다면 환경 차단 사유
6. 실행한 테스트와 결과
