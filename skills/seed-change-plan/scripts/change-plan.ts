...
const SURFACE_RULES: Array<{ kind: SurfaceKind; pattern: RegExp }> = [
  {
    kind: "generated",
    pattern:
      /^(?:packages\/rootage\/(?:__generated__\/|components\/schema\.json$)|packages\/(?:lynx-)?qvism-preset\/src\/(?:vars\/|token\.css$|tokens\.ts$)|packages\/css\/(?:recipes\/|vars\/|[^/]+\.css$)|packages\/lynx-css\/(?:recipes\/|vars\/|[^/]+\.css$)|docs\/public\/__registry__\/)|(?:^|\/)(?:dist|node_modules)(?:\/|$)/,
  },
