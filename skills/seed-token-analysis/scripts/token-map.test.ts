...
describe("seed-token-analysis token map", () => {
  it("현재 체크아웃에서 색상 토큰의 alias, 사용처와 생성 표면을 연결한다", async () => {
    const result = await mapSeedColorToken("fg/neutral-inverted");

    expect(result.token).toMatchObject({
      canonical: "$color.fg.neutral-inverted",
      state: "matched",
      publicNames: {
        cssVariable: "--seed-color-fg-neutral-inverted",
        tailwind: "fg-neutral-inverted",
      },
    });
    ...
    expect(result.resolvedValues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          mode: "theme-light",
          resolved: "#ffffff",
          chain: ["$color.fg.neutral-inverted", "$color.palette.gray-00"],
        }),
      ]),
    );
    ...
    expect(palette.dependentTokens).toEqual(
      expect.arrayContaining([
        {
          token: "$color.fg.neutral-inverted",
          modes: ["theme-light"],
          path: "packages/rootage/color.yaml",
        },
      ]),
