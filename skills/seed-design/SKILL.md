...
뒤에 `제목 — description — whenToRead`가 붙습니다. 마지막 조각은 그 문서를 언제 읽으면 되는지
알려줍니다. 이 정보로 읽을 문서를 고른 다음, 그 줄의 첫 필드를 `read`에 넘깁니다. `grep`으로
줄을 걸러도 설명이 함께 남고, `awk '{print $1}'`로 주소만 잘라낼 수 있습니다.
