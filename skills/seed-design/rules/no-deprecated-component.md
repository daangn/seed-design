...
전체·플랫폼 인덱스 또는 선택된 플랫폼 registry에서 현재 유효한 deprecation 출처와 대상 버전을 확인할 수 있을 때 적용합니다. 다른 플랫폼의 목록을 이식하지 않습니다.

- 인덱스를 정상적으로 읽었는데 출처가 없음 → `not-applicable`
- 인덱스·연결 문서·registry를 읽지 못함 → `not-verified`
