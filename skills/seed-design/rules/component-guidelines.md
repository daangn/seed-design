...
1. **Do/Dont 기준** — `body="…"` 속성 전부. 하나가 기준 하나입니다.
   ```bash
...
**이 목록이 바닥(floor)입니다 — 전부 verdicts에 나와야 하고, 하나라도 건너뛰면 그 실행은 불완전합니다.** 개수가 `coverage.expected`가 됩니다. 내용이 겹쳐 보여도 합치지 않습니다 — 판단이 끼는 순간 id가 흔들립니다. 중복이면 둘 다에 같은 판정을 적으면 됩니다. `DoImage body="~할 수 있습니다"` 같은 허용문도 목록에는 남기되 판정을 `pass`(허용문 — 위반 불성립)로 적습니다.
