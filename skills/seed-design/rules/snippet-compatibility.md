...
유효한 snippet path 아래에 `@file` 헤더가 있는 설치 스니펫이 있을 때 적용합니다.

- 설정·스니펫이 없음 → `not-applicable`
- 헤더가 없어 설치 항목을 식별할 수 없음 → `not-verified`
...
호환 검사는 읽기 전용으로만 실행합니다. 패키지 설치, snippet add·upgrade 같은 변경 명령은 실행하지 않고 finding의 수정 프롬프트로만 제시합니다.
