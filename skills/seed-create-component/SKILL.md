...
다음 조건을 모두 만족하는 경우에만 [`seed-orchestrate-component`](../seed-orchestrate-component/SKILL.md)를 사용한다.

- 두 개 이상의 독립적인 쓰기 범위 또는 검증 관점이 있다.
- Registry·예제·생성물처럼 레이어 사이의 소비 계약을 확인해야 한다.
- 사용자 요구가 상태·모션·성능·플랫폼 차이를 함께 포함한다.
...
협업을 사용하면 각 담당의 작업 종료를 전체 완료로 보지 않는다. 통합 담당이 원천부터 소비 경로를 연결한 뒤 [`검증 체크리스트`](references/verification-checklist.md)를 독립 검증 담당이 실행한다. 검증 담당은 구현하지 않으며, 환경별 통과·실패·환경 차단·미확인을 분리한다.

단독 작업에서의 기존 검증·changeset·제출 규칙은 바꾸지 않는다.
