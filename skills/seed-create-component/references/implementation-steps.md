...
전체 문서 빌드를 기본 장면의 선행 조건으로 두지 않는다. Lynx 기본 장면은 필요한 package·생성물과 대상 native bundle을 실제 공개 소비 경로로 연결해 먼저 확인한다. 명령 범위는 현재 빌드 도구가 지원하는 것으로 정하고, 대상별 빌드 옵션을 임의로 만들지 않는다. 문서 페이지 자체가 검증 대상이면 필요한 문서 출력을 준비한다. 관련 전체 문서 검증은 시나리오 구현이 모인 안정된 차수에서 수행하며, 작은 원천 수정마다 무조건 반복하지 않는다.
