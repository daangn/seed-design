...
이 분기는 `package.json`의 dependency 또는 peer dependency 범위를 수정하지 않는다. 새 기능 때문에 필요한 peer dependency 하한 조정은 Version Changes PR에서 수동으로 처리한다.
...
`--base-ref`에는 [변경 계획](impact-plan.md)이 선택한 실제 기준 브랜치를 지정한다. 리베이스 전에는 feature 브랜치의 기존 기준을 사용한다. 리베이스 뒤 최종 확인에서는 확정된 `targetRef`를 사용한다.
...
[version-matrix.md](version-matrix.md)를 읽는다. 후보별 공개 export, 타입, prop, Recipe, 토큰, 렌더 결과, 접근성 동작을 직접 확인하고 `major`, `minor`, `patch`, `안함(제외)` 중 하나를 추천한다.
...
[patterns.md](patterns.md)를 읽고 디자인 시스템 소비자 관점의 한국어 메시지를 작성한다. 하나의 사용자 변화가 여러 패키지에 전파되면 한 파일에 묶는다. 서로 독립적인 변화는 파일을 나눈다.
...
가장 높은 확정 bump는 최종 [변경 계획](impact-plan.md)의 `--bump` 근거로 보고한다. changeset 승인은 commit·push·PR 승인이 아니며, 이 분기에서는 rebase, commit, push, PR 생성, 배포를 수행하지 않는다.
