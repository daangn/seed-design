...
공개 패키지는 아래 SemVer 분류를 기본으로 사용한다. 아직 `0.x`인 Lynx 패키지는 아래의 초기 개발 정책을 우선 적용한다.
...
Lynx 패키지는 안정화 전까지 `0.x`를 유지한다. 이 기간에는 기능 추가와 breaking change를 `minor`, 호환되는 버그 수정을 `patch`로 배포한다. 아래 표와 변경 전파 규칙의 `major`도 해당 Lynx 패키지가 `0.x`인 동안에는 `minor`로 적용하되, breaking change의 변경 내용과 마이그레이션 안내는 생략하지 않는다. `1.0.0` 전환은 별도로 명시적으로 결정한다.

새 Lynx 공개 패키지는 manifest의 `0.0.0`과 첫 `minor` changeset으로 `0.1.0`에서 시작한다. 그 외 새 공개 패키지는 `1.0.0`에서 시작한다. 기존 패키지에 하위 호환 컴포넌트나 API를 추가하는 경우에는 해당 패키지를 `minor`로 올린다.
