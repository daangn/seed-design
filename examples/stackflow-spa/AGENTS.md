...
- 신규 activity는 NAS만 쓴다. NAS가 기본이므로 이름에 표식을 붙이지 않고, 구형 AppScreen을 쓰는 화면만 `ActivityLegacy*` 로 이름 짓고 라우트도 `/legacy-*` 로 둔다. 구형 화면은 아래 6개뿐이고 전부 AppBar title에 `Legacy` 표식이 붙어 있다. `ActivityHome` 링크 제목에도 `[Legacy]` 를 단다. 표식 없는 항목은 NAS라는 뜻이므로, 구형을 새로 추가하거나 옮길 때 이름·라우트·표식을 같이 옮긴다.
...
| --- | --- | --- | --- |
| A | `seed-design/ui/app-screen` (vendored snippet) | `ActivityLegacyAppScreen`, `ActivityLegacyAnimateFalseTest`, `ActivityLegacyAppScreenIntersectionObserver` | 회귀 검증용. `ActivityLegacyAppScreen` 이 화면·AppBar 표면을 한 화면에서 전부 토글하고, 나머지 둘은 거기 담을 수 없는 케이스만 남긴다. NAS 짝은 각각 `ActivityAppScreen`, `ActivityAnimateFalseTest`, `ActivityAppScreenIntersectionObserver` |
| B | `@seed-design/stackflow` 직접 import | `ActivityLegacyPullToRefreshPreview`, `ActivityLegacyPullToRefreshPreventPull` | snippet `AppScreenContent`가 PTR을 이미 통합하고 있어서, 수동 조합을 보여주려고 snippet을 우회한다. NAS 짝은 `ActivityPullToRefreshPreview` / `ActivityPullToRefreshPreventPull`이며 snippet의 `ptr` prop을 쓴다 |
