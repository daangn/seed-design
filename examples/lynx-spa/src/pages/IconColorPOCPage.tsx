...
const VARIANT_FG_CLASS = {
  brandSolid: "text-fg-on-brand-solid",
  criticalSolid: "text-fg-on-critical-solid",
  neutralWeak: "text-fg-neutral",
} as const;
    ...
    <view>
      ...
      >
        ...
        <IconPlusFill
          ...
          size={32}
        />
        <text className={clsx("t2-regular", VARIANT_FG_CLASS[variant])}>current: {variant}</text>
      </view>
    ...
    <scroll-view scroll-y className="flex flex-col gap-x4 flex-1">
      ...
      <view className="mb-x4">
        <SectionHeader
          ...
          desc="아이콘이 렌더되는지 자체부터 좌표화. 왼쪽: IconPlusFill with fg.on-brand-solid. 오른쪽: raw <image> + data URI + semantic tint. 둘 중 어느 쪽이 보이는지."
        />
        <BrandBg>
          <IconPlusFill color="var(--seed-color-fg-on-brand-solid)" size={32} />
          <image
            src={PLUS_ICON_DATA_URI}
            {...{ "tint-color": "var(--seed-color-fg-on-brand-solid)" }}
            className="w-x8 h-x8"
          />
          <text className="t2-regular text-fg-on-brand-solid">좌: 컴포넌트 / 우: data URI</text>
        </BrandBg>
      </view>
      ...
      <view className="mb-x4">
        <SectionHeader
          ...
          desc='<image tint-color="var(--seed-color-fg-on-brand-solid)" /> — 흰 아이콘이면 Lynx attribute parser 가 CSS var 해석.'
        />
        <BrandBg>
          {/*
          ...
          */}
          <IconPlusFill color="var(--seed-color-fg-on-brand-solid)" size={32} />
          <text className="t2-regular text-fg-on-brand-solid">
            ✅ 흰색 → var() 해석됨 / ❌ 검정 → 미해석
          </text>
        </BrandBg>
      </view>
