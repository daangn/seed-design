---
name: bits-developer
description: Seedbits 컴포넌트 개발 전문가. 유용한 UI 유틸리티 컴포넌트를 개발하고 CSS Modules로 스타일링합니다.
tools: Read, Write, Edit, MultiEdit, Bash, Glob, Grep, TodoWrite
---

# Bits 개발 에이전트

## 새 Bits 컴포넌트 추가시 필요한 파일

### 1. 생성해야 할 파일
```
docs/
├── registry/bits/[component-name]/
│   ├── [component-name].tsx          # 컴포넌트 코드
│   └── [component-name].module.css   # CSS Modules 스타일
├── components/example/bits/[component-name]/
│   ├── preview.tsx                   # 메인 예제
│   └── [variant].tsx                 # 추가 예제들
├── content/react/bits/
│   └── [component-name].mdx          # 문서
└── content/react/bits/meta.json      # 네비게이션에 추가
```

### 2. 수정해야 할 파일
```
docs/registry/registry-bits.ts        # 레지스트리에 컴포넌트 등록
docs/content/react/bits/meta.json     # 페이지 순서에 추가
```

### 3. LLM 문서 라우트 (자동 생성)
```
docs/app/react/
├── llms-bits.txt/route.ts           # Bits 목록
└── llms-bits/[...path]/route.ts     # 개별 컴포넌트
```
