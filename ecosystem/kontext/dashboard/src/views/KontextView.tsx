...
import { useHighlightedPaths } from "@/hooks/useHighlightedPaths";
import { useFileContent } from "@/hooks/useFileContent";
import { extToLang } from "@/hooks/useShiki";
import { getPackageDotColor, getPackageLabel } from "@/lib/packages";
import { cn } from "@/lib/utils";
import type { KontextGraph } from "@/types";

interface KontextViewProps {
  graph: KontextGraph;
  onSaved: () => void;
}
...
export function KontextView({ graph, onSaved }: KontextViewProps) {
  ...
  const [previewTab, setPreviewTab] = useState<"yaml" | "file">("yaml");
  const [selectedFilePath, setSelectedFilePath] = useState<string | null>(null);

  // Dependency highlight map for the file tree
  const highlightedPaths = useHighlightedPaths(relations, graph, selectedPackage);

  // File content for preview
  const fileContent = useFileContent();
  const [yamlPanelWidth, setYamlPanelWidth] = useState(660);
  ...
  return (
    <div className="flex h-full">
      ...
      >
        <Tabs defaultValue="kontexts" className="flex flex-col h-full">
          <div className="flex items-center justify-between px-3 pt-2.5 pb-1">
            <TabsList className="h-7 bg-transparent p-0 gap-1">
              <TabsTrigger
                value="kontexts"
                className="text-[10px] h-6 px-2 data-[state=active]:bg-accent"
              >
                Kontexts
              </TabsTrigger>
              <TabsTrigger
          ...
          {/* KONTEXTS tab */}
          <TabsContent value="kontexts" className="flex-1 min-h-0 mt-0 overflow-auto">
          ...
          <TabsContent value="files" className="flex-1 min-h-0 mt-0 overflow-hidden">
            <FileTree
              ...
              activeKontextDir={selectedPackage}
              onFileSelect={(path) => {
                if (path.includes(".")) {
                        ...
                        {rel.affects.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {rel.affects.map((affect) => {
                              const affectExists = affect.path
                                ? graph.nodes.some((n) => n.id === affect.path && n.exists)
                                : false;
                              return (
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <button
                                          ...
                                          onClick={() => {
                                            if (affect.path) setScrollToPath(affect.path);
                                          }}
                                        >
                                          {affectExists ? (
                                      ...
                                      <TooltipContent className="text-[11px]">
                                        {affectExists
                                          ? "File exists - click to find in tree"
                                          : affect.optional
                                            ? "Optional - may not need updating"
                                            : affect.path
      ...
      <ResizeHandle
        direction="left"
        onResize={(delta) => setYamlPanelWidth((w) => Math.max(280, Math.min(720, w + delta)))}
      />

      {/* Right: Preview panel (YAML / File tabs) */}
      <div
      ...
      >
        <Tabs
          value={previewTab}
          onValueChange={(v) => setPreviewTab(v as "yaml" | "file")}
        ...
        >
          <div className="flex items-center justify-between px-3 py-2 border-b border-border">
            <TabsList className="h-7 bg-transparent p-0 gap-1">
              ...
              <TabsTrigger
                ...
                className="text-[10px] h-6 px-2 data-[state=active]:bg-accent"
              >
                {selectedFilePath ? selectedFilePath.split("/").pop() : "File"}
              </TabsTrigger>
            </TabsList>
            ...
            {previewTab === "file" && selectedFilePath && (
          ...
          <TabsContent value="yaml" className="flex-1 min-h-0 mt-0">
            {selectedPackage ? (
              <CodePreview code={yamlPreview} lang="yaml" />
            ) : (
          ...
          <TabsContent value="file" className="flex-1 min-h-0 mt-0">
            {fileContent.loading ? (
              <div className="p-4 text-[11px] text-muted-foreground/40 animate-pulse">
                Loading...
              </div>
            ) : fileContent.error ? (
              <div className="p-4 text-[11px] text-muted-foreground/50">{fileContent.error}</div>
            ) : fileContent.fileContent ? (
              <CodePreview
                code={fileContent.fileContent.content}
                lang={extToLang(fileContent.fileContent.extension)}
              />
            ) : (
              <div className="p-4 text-[11px] text-muted-foreground/40">
