...
- **인자를 고쳐서 받지 않는다.** `--help`에 적힌 형식과 다른 값은 없는 값과 똑같이 답하고, 올바른 형식은 `--help`의 설명과 예시만으로 알 수 있게 쓴다. 예: `seed-design docs list /react/`를 `react`로 읽지 않고, `compat " ui:action-button"`의 공백을 떼지 않는다. `docs read`가 주소 뒤의 `#앵커`를 무시하는 것은 고치기가 아니라, URL fragment와 같은 뜻이라서 `--help`에 적어 둔 문법이다.
...
- `0`은 명령이 답을 냈다는 뜻이고 답이 판정이면 좋은 쪽이다. `1`은 명령이 정상적으로 돌았지만 답이 부정적이라는 뜻이며, `2`는 답을 내지 못했다는 뜻이다. 네트워크 장애와 「검사해서 문제를 찾았다」를 한 코드로 묶으면 CI가 전자를 후자로 읽는다.
- `1`은 보고할 판정이 있는 명령만 쓴다. `compat`이 호환되지 않는 항목을 찾았을 때, `docs search`에 걸리는 문서가 없을 때, `docs list`에 없는 섹션을 넣었을 때 셋뿐이다. `init`·`add`·`add-all`은 판정이 없으므로 `0`과 `2`만 쓴다.
...
프롬프트가 있는 자리와 각각의 비TTY 결말은 다음과 같다.
...
`seed-design.json`이 없을 때 외부 명령(`seed-design init`)을 `execa`로 재호출하지 않고 `src/utils/init-config.ts`로 직접 만든다. 기본값은 `rsc=false`, `tsx=true`, `framework="react"`, `path="./seed-design"`, `telemetry=true`이다. 프로젝트 의존성을 보고 React와 Lynx 중 하나를 고르는 `detectFramework(cwd)`는 `init`만 쓰고, 이 경로는 `framework="react"`를 그대로 쓴다. `add`에는 `--yes`를 두지 않는다. 설정 파일을 만드는 일은 `init`이 갖고 있고, 기본값을 대신 고르지 않는 이유는 「비대화형 실행」에 있다.
...
- 주소는 문서 인덱스의 `docUrl` 그대로다. `category/section/item`으로 재조립하면 섹션 그룹핑보다 깊은 문서의 중간 slug가 사라지고 서로 다른 두 문서가 같은 주소를 갖는다.
- `docs read`는 `docs list`·`docs search`가 출력한 주소와 정확히 같을 때만 읽는다. 짧은 이름으로 찾지 않는 이유는 여러 문서 경로가 다른 문서 경로의 꼬리여서, 짧은 이름이 여러 문서에 걸리거나 사이트가 자라면서 다른 문서를 가리키게 되기 때문이다.
- `docs read`는 주소 뒤의 `#앵커`를 무시한다. `docs search`는 제목에서 걸린 문서에 앵커를 붙여 출력하고, 사이트에서 복사한 URL에도 앵커가 붙는다. 앵커는 URL fragment처럼 문서 안의 위치일 뿐이라 같은 문서를 연다.
- `docs list`는 섹션 id만 받고, 생략하면 모든 문서를 나열한다. 인덱스에 선언된 묶음이 섹션뿐이라서, 그 아래 범위를 받으려면 CLI가 주소에서 묶음을 계산해 내야 한다. 섹션 하나는 한 번에 나열해도 될 크기라 그대로 나열하고, 더 좁히려면 `grep`으로 거른다. 섹션 목록만 내는 모양은 두지 않아서, 섹션을 넣든 생략하든 같은 줄 형식으로 답한다.
...
- `docs search`는 문서 사이트 검색창과 같은 색인으로 본문까지 찾는다. 검색 색인은 주소만 돌려주므로, 제목과 description은 문서 인덱스에서 주소로 찾아 붙인다.
- `docs list`와 `docs search`는 항목 하나를 한 줄에 쓰고, 줄의 첫 필드를 공백 없는 주소로 둔다. 문서 줄에는 주소 뒤에 `제목 — description`이 붙는다. 에이전트가 `grep`으로 줄을 거르고 `awk '{print $1}'`로 주소를 잘라 쓰기 때문이다. 설명을 다음 줄로 넘기면 `grep`이 설명을 걸러내고, 주소가 첫 필드가 아니면 잘라낼 수 없다. 문서 정보를 더 싣게 되더라도 같은 줄 끝에 잇는다.
