...
- `docs read`는 `docs list`·`docs search`가 출력한 주소와 정확히 같을 때만 읽는다. 짧은 이름으로 찾지 않는 이유는 문서 경로 51개가 다른 문서 경로의 꼬리여서, 짧은 이름이 여러 문서에 걸리거나 사이트가 자라면서 다른 문서를 가리키게 되기 때문이다.
...
- `docs list`는 섹션 id만 받고, 생략하면 모든 문서를 나열한다. 인덱스에 선언된 묶음이 섹션뿐이라서, 그 아래 범위를 받으려면 CLI가 주소에서 묶음을 계산해 내야 한다. 가장 큰 섹션도 100여 개라 한 번에 나열하고, 더 좁히려면 `grep`으로 거른다. 섹션 목록만 내는 모양은 두지 않아서, 섹션을 넣든 생략하든 같은 줄 형식으로 답한다.
