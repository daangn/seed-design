...
- `docs search`는 문서 사이트 검색창과 같은 색인으로 본문까지 찾는다. 검색 색인은 주소만 돌려주므로, 제목·description·whenToRead는 문서 인덱스에서 주소로 찾아 붙인다.
- `docs list`와 `docs search`는 항목 하나를 한 줄에 쓰고, 줄의 첫 필드를 공백 없는 주소로 둔다. 문서 줄에는 주소 뒤에 `제목 — description — whenToRead`가 붙고, 인덱스에 없는 조각은 건너뛴다. `whenToRead`에는 라벨을 붙이지 않는다. 문구가 「~할 때 읽는다」로 끝나서 라벨 없이도 뜻이 드러난다. 에이전트가 `grep`으로 줄을 거르고 `awk '{print $1}'`로 주소를 잘라 쓰기 때문이다. 설명을 다음 줄로 넘기면 `grep`이 설명을 걸러내고, 주소가 첫 필드가 아니면 잘라낼 수 없다. 문서 정보를 더 싣게 되더라도 같은 줄 끝에 잇는다.
