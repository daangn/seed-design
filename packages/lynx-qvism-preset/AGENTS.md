...
- `inset` / `inset-*` shorthand는 Lynx preset source에서 사용하지 않는다. 전체 영역 채움은 `top` / `right` / `bottom` / `left` longhand로 작성하고, Lightning CSS 이후 재생성되는 `inset`은 preset의 final CSS post-transform에서만 확장한다.
