...
- 06307f2: `AlertDialog`, `MenuSheet` 등 Dialog 기반 컴포넌트가 열릴 때 첫 항목 대신 컨테이너에 포커스가 가도록 수정합니다.
...
- faf19f3: HelpBubble이 좁은 화면에서 화면 밖으로 잘리던 문제를 수정합니다. (Popover의 floating 요소 너비를 viewport에 맞게 동적으로 제한하고, 긴 텍스트가 Help Bubble 밖으로 넘치지 않도록 긴 단어 중간 줄바꿈을 허용합니다.)

## 1.1.25

### Patch Changes

- d71e6db: `@seed-design/stackflow` v1.1.22 이상 (또는 dev 라인 v1.2.11 이상)의 WAAPI 기반 AppScreen 전환 코드와 호환되도록 transition CSS animation selector를 제거하고 AppBar background slot 스타일을 추가합니다. (PR #1444 백포팅)
