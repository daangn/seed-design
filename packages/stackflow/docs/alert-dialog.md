file: stackflow/alert-dialog.mdx
...
## Making an Alert Dialog Activity \[#making-an-alert-dialog-activity]
...
<StackflowExample path="/alert-dialog-activity">
  ```tsx title='ActivityAlertDialogActivity.tsx'
  ...
  ```tsx title='ActivityAlertDialog.tsx'
...
2. `onOpenChange`를 통해 Alert Dialog가 닫힐 때 `pop()`을 실행하여 Activity를 종료합니다.
...
#### Keeping Alert Dialog Mounted \[#keeping-alert-dialog-mounted]
...
1. `open`을 `transitionState`로 관리하여 다른 Activity가 위에 push되어도 Alert Dialog가 unmount되지 않도록 합니다.
2. `modal={isActive}`로 Alert Dialog Activity가 비활성 상태일 때 `modal`을 `false`로 설정합니다. 이렇게 하지 않으면, 위에 push된 Activity에서 포커스 및 스크린 리더 접근이 동작하지 않습니다.
3. `onOpenChange` 핸들러에서 `isActive`인 경우에만 `pop()`을 실행하여, 비활성 상태에서의 의도치 않은 Activity 종료를 방지합니다.

## Syncing Alert Dialog State with a Step \[#syncing-alert-dialog-state-with-a-step]
...
<StackflowExample path="/alert-dialog-step">
  ```tsx title='ActivityAlertDialogStep.tsx'
