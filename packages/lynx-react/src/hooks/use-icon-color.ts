...
// resolved hex 를 돌려주므로 main-thread 에서 읽어 `tint-color` 로 mirror 한다.
function syncTintColor(ref: RefObject<IconElement>) {
  ...
  if (typeof requestAnimationFrame === "function") {
    requestAnimationFrame(syncTintColorOnce);
  }
}

/**
 * Lynx `<image>` 의 tint color 를 recipe 의 CSS `color` 로부터 main-thread 에서 읽어
 * `tint-color` attribute 로 mirror. `deps` 가 바뀌면 즉시 한 번, className 반영이
 * 끝난 다음 프레임에 한 번 더 재동기화.
 *
 * ```tsx
