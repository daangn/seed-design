...
컴포넌트를 추가하거나 여러 레이어(rootage 스키마 → qvism recipe → react 패키지 → 문서)에 걸친 변경을 할 때는 레포에 포함된 `seed-component` 스킬을 참고해 주세요. 플랫폼 결정, 카테고리별 패턴, 작업 순서와 검증까지 안내합니다.

Claude Code를 쓰신다면 클론 직후 `/seed-component`로 바로 사용할 수 있습니다. `skills/`가 원천이고 `.claude/skills`가 이 디렉터리로의 심링크입니다.

## changeset

사용자에게 보이는 변경(기능 추가, 버그 픽스, 스타일 변경)에는 changeset이 필요합니다. `.changeset/*.md`를 직접 작성하지 말고 `seed-change` 스킬의 changeset 분기를 사용해 주세요. 변경된 패키지 후보를 찾고 bump와 유저향 메시지를 확정합니다. Claude Code에서는 `/seed-change`입니다.
...
다른 곳에서 가져온 코드나 에셋이 포함된다면 **출처와 라이선스를 PR 본문에 적어 주세요.** 원본에 저작권 고지가 붙어 있다면 지우지 말고 소스 파일에 그대로 남겨 주세요. PR 본문은 배포물에 포함되지 않습니다. 출처를 확인할 수 없으면 머지할 수 없습니다.

AI 도구로 작성한 코드는 본인 기여로 봅니다. 별도로 밝히지 않아도 됩니다.
