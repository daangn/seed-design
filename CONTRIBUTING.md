...
본인이 직접 작성하지 않은 코드나 에셋이 포함된다면 **출처와 라이선스를 PR 본문에 적어 주세요.** 원 라이선스가 저작권 고지 유지를 요구한다면 그 고지는 소스 파일에 그대로 남겨 주세요. PR 본문은 배포물에 포함되지 않습니다. `NOTICE` 갱신이 필요한지는 리뷰에서 함께 확인합니다. 출처를 확인할 수 없으면 머지할 수 없습니다.
